...
class CanvasExtraState {
    ...
    getPathBoundingBox(pathType = PathType.FILL, transform) {
    ...
    getClippedPathBoundingBox(pathType = PathType.FILL, transform) {
        return Util.intersect(this.clipBox, this.getPathBoundingBox(pathType, transform));
    }
}
function putBinaryImageData(ctx, imgData, transferMaps) {
...
export class CanvasGraphics {
    ...
    _createMaskCanvas(img) {
        ...
        if ((img.bitmap || img.data) && img.count > 1) {
            ...
            if (cachedImage && !isPatternFill) {
                const offsetX = Math.round(Math.min(currentTransform[0], currentTransform[2]) +
                    currentTransform[4]);
                const offsetY = Math.round(Math.min(currentTransform[1], currentTransform[3]) +
                    currentTransform[5]);
                return {
                    canvas: cachedImage,
                    offsetX,
    ...
    // Path
    [OPS.constructPath](ops, args, minMax) {
        const ctx = this.ctx;
    ...
    [OPS.stroke](consumePath = true) {
        ...
        if (this.contentVisible) {
            if (typeof strokeColor === "object" && strokeColor?.getPattern) {
                ctx.save();
                ctx.strokeStyle = strokeColor.getPattern(ctx, this, ctx.mozCurrentTransformInverse, PathType.STROKE);
    ...
    [OPS.fill](consumePath) {
        ...
        if (isPatternFill) {
            ctx.save();
            ctx.fillStyle = fillColor.getPattern(ctx, this, ctx.mozCurrentTransformInverse, PathType.FILL);
            needRestore = true;
        }
    ...
    [OPS.showText](glyphs) {
        ...
        if (current.patternFill) {
            ctx.save();
            const pattern = current.fillColor.getPattern(ctx, this, ctx.mozCurrentTransformInverse, PathType.FILL);
        ...
        for (i = 0; i < glyphsLength; ++i) {
            ...
            if (font.remeasure && width > 0) {
                ...
                const measuredWidth = ((ctx.measureText(character).width * 1000) / fontSize) *
                    fontSizeScale;
    ...
    [OPS.shadingFill](objId) {
        ...
        ctx.fillStyle = pattern.getPattern(ctx, this, ctx.mozCurrentTransformInverse, PathType.SHADING);
    ...
    [OPS.beginAnnotation](id, rect, transform, matrix, hasOwnCanvas) {
        ...
        if (Array.isArray(rect) && rect.length === 4) {
            ...
            if (hasOwnCanvas && this.annotationCanvasMap) {
                ...
                rect[2] = width;
                rect[3] = height;
                const [scaleX, scaleY] = Util.singularValueDecompose2dScale(this.ctx.mozCurrentTransform);
