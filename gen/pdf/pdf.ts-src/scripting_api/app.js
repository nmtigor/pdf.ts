 ...
 *
 * @module pdf/pdf.ts-src/scripting_api/app.ts
 * @license Apache-2.0
 ******************************************************************************/
...
export class App extends PDFObject {
    ...
    get fs() {
        if (this._fs === undefined) {
            this._fs = new Proxy(new FullScreen({ send: this._send }), this._proxyHandler);
        }
        return this._fs;
    }
    set fs(_) {
    ...
    get thermometer() {
        if (this._thermometer === undefined) {
            this._thermometer = new Proxy(new Thermometer({ send: this._send }), this._proxyHandler);
        }
