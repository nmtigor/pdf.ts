...
export class AForm {
    ...
    _dateActionsCache = new Map();
    /**
     * The e-mail address regex below originates from:
     * https://html.spec.whatwg.org/multipage/input.html#valid-e-mail-address
     */
    _emailRegex = new RegExp("^[a-zA-Z0-9.!#$%&'*+\\/=?^_`{|}~-]+" +
    ...
    _tryToGuessDate(cFormat, cDate) {
        ...
        if (!actions) {
            ...
            cFormat.replaceAll(/(d+)|(m+)|(y+)|(H+)|(M+)|(s+)/g, (match, d, m, y, H, M, s) => {
                if (d) {
                    actions.push((n, date) => {
                        if (n >= 1 && n <= 31) {
                            date.setDate(n);
    ...
    AFNumber_Keystroke(nDec, /* unused */ sepStyle, negStyle, /* unused */ currStyle, /* unused */ strCurrency, /* unused */ bCurrencyPrepend) {
        const event = globalThis.event;
