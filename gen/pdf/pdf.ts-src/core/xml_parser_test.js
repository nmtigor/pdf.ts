...
console.log("%c>>>>>>> test searchNode() >>>>>>>", `color:${css_1}`);
{
    ...
    <a>
        ...
        const root = new SimpleXMLParser({ hasAttributes: true }).parseFromString(xml).documentElement;
    ...
    <a>
        ...
        const buffer = [];
        root.dump(buffer);
        console.assert(buffer.join("").replace(/\s+/g, "") === xml.replace(/\s+/g, ""));
    }
...
//# sourceMappingURL=xml_parser_test.js.map