...
import { MOZCENTRAL, PDFJSDev, TESTING } from "../../../global.js";
import { AnnotationActionEventType, AnnotationBorderStyleType, AnnotationEditorType, AnnotationFieldFlag, AnnotationFlag, AnnotationReplyType, AnnotationType, BASELINE_FACTOR, FeatureTest, getModificationDate, IDENTITY_MATRIX, info, LINE_DESCENT_FACTOR, LINE_FACTOR, OPS, RenderingIntentFlag, shadow, stringToPDFString, Util, warn, } from "../shared/util.js";
...
export class AnnotationFactory {
     ...
     * @private
     */
    static _create(xref, ref, annotationGlobals, idFactory, 
    // acroForm: Dict | undefined,
    // attachments: Attachments | undefined,
    // xfaDatasets: DatasetReader | undefined,
    collectFields = false, pageIndex = undefined, pageRef = undefined) {
        ...
        for (const annotation of annotations) {
            ...
            switch (annotation.annotationType) {
                case AnnotationEditorType.FREETEXT:
                    ...
                    promises.push(FreeTextAnnotation.createNewAnnotation(xref, annotation, dependencies, { evaluator, task, baseFontRef }));
                    break;
            ...
            switch (annotation.annotationType) {
                ...
                case AnnotationEditorType.HIGHLIGHT:
                    if (annotation.quadPoints) {
                        ...
                        promises.push(InkAnnotation.createNewPrintAnnotation(annotationGlobals, xref, annotation, { evaluatorOptions: options }));
                    }
                    break;
                case AnnotationEditorType.INK:
...
export class Annotation {
    ...
    constructor(params) {
        ...
        const MK = dict.get("MK"); // Table 187
    ...
    async getOperatorList(evaluator, task, intent, annotationStorage) {
        ...
        let optionalContent;
        if (this.oc) {
            optionalContent = await evaluator.parseMarkedContentProps(this.oc, 
            /* resources = */ undefined);
        }
...
export class MarkupAnnotation extends Annotation {
    ...
    static createNewDict(annotation, xref, _) {
    ...
    /** @final */
    setDefaultAppearance$({ xref, extra, strokeColor, fillColor, blendMode, strokeAlpha, fillAlpha, pointsCallback, }) {
...
export class WidgetAnnotation extends Annotation {
    ...
    getBorderAndBackgroundAppearances(annotationStorage) {
        ...
        if (this.backgroundColor) {
            str = `${getPdfColor(this.backgroundColor, 
            /* isFill */ true)} ${rect} f `;
        }
        if (this.borderColor) {
            const borderWidth = this.borderStyle.width || 1;
            str += `${borderWidth} w ${getPdfColor(this.borderColor, 
            /* isFill */ false)} ${rect} S `;
        }
        return str;
    }
    ...
    async save(evaluator, task, annotationStorage) {
        ...
        if (!this._needAppearances) {
            appearance = await this.getAppearance$(evaluator, task, RenderingIntentFlag.SAVE, annotationStorage);
    ...
    _getMultilineAppearance(defaultAppearance, lines, font, fontSize, width, height, alignment, hPadding, vPadding, descent, lineHeight, AnnotStorageRecord) {
        return "";
    }
    ...
    async getAppearance$(evaluator, task, intent, annotationStorage) {
        ...
        if (encodingError && this._isOffscreenCanvasSupported) {
            ...
            font = await WidgetAnnotation._getFontData(evaluator, task, { fontName, fontSize: 0 }, resources);
            ...
            [defaultAppearance, fontSize, lineHeight] = this.computeFontSize$(totalHeight - 2 * defaultPadding, totalWidth - 2 * defaultHPadding, value, font, lineCount);
        ...
        if (alignment === 0 || alignment > 2) {
            ...
            return (`/Tx BMC q ${colors}BT ` +
                defaultAppearance +
                ` 1 0 0 1 ${numberToString(defaultHPadding)} ${numberToString(bottomPadding)} Tm (${escapeString(encodedLines[0])}) Tj` +
                " ET Q EMC");
        }
    ...
    static async _getFontData(evaluator, task, appearanceData, resources) {
        ...
        await evaluator.handleSetFont(resources, [fontName && Name.get(fontName), fontSize], 
    ...
    _renderText(text, font, fontSize, totalWidth, alignment, prevInfo, hPadding, vPadding) {
        ...
        return `${shiftStr} ${numberToString(vPadding)} Td (${escapeString(text)}) Tj`;
    }
...
class TextWidgetAnnotation extends WidgetAnnotation {
    ...
    _getCombAppearance(defaultAppearance, font, text, fontSize, width, height, hPadding, vPadding, descent, lineHeight, annotationStorage) {
        ...
        return (`/Tx BMC q ${colors}BT ` +
            defaultAppearance +
            ` 1 0 0 1 ${numberToString(hPadding)} ${numberToString(vPadding + descent)} Tm ${renderedComb}` +
            " ET Q EMC");
    }
    ...
    async getAppearance$(evaluator, task, intent, annotationStorage) {
        ...
        if (!fontSize) {
            ...
            for (const { displayValue } of this.data.options) {
                const width = this.getTextWidth$(displayValue, font);
                if (width > lineWidth) {
                    lineWidth = width;
                    value = displayValue;
                }
            }
            [defaultAppearance, fontSize] = this.computeFontSize$(lineHeight, totalWidth - 2 * defaultHPadding, value, font, -1);
        }
        else {
...
class FreeTextAnnotation extends MarkupAnnotation {
    ...
    constructor(params) {
        ...
        if (this._hasAppearance) {
            const { fontColor, fontSize } = parseAppearanceStream(this.appearance, evaluatorOptions, xref);
            ...
            if (this._contents.str) {
                ...
                const { coords, bbox, matrix } = FakeUnicodeFont.getFirstPositionInfo(this.rectangle, this.rotation, fontSize);
...
class LineAnnotation extends MarkupAnnotation {
    constructor(params) {
        ...
        if (!this.appearance) {
            ...
            const interiorColor = getRgbColor(dict.getArray("IC"));
...
class InkAnnotation extends MarkupAnnotation {
    ...
    static async createNewAppearanceStreamForHighlight(annotation, xref, params) {
        const { color, rect, outlines: { outline }, opacity, } = annotation;
        const appearanceBuffer = [
            `${getPdfColor(color, /* isFill */ true)}`,
            "/R0 gs",
        ];
...
class StampAnnotation extends MarkupAnnotation {
    ...
    static async createImage(bitmap, xref) {
        ...
        const hasAlpha = buf32.some(FeatureTest.isLittleEndian
            ? (x) => x >>> 24 !== 0xff
            : (x) => (x & 0xff) !== 0xff);
        if (hasAlpha) {
