 ...
 */
function generateFont({ alias, local, path, fallback, style, ultimate }, src, localFontPath, useFallback = true, usePath = true, append = "") {
    const result = {};
    if (local) {
    ...
    if (alias) {
        ...
        Object.assign(result, generateFont(substitution, src, localFontPath, 
        /* useFallback = */ useFallback && !fallback, 
        /* usePath = */ usePath && !path, aliasAppend));
    }
    if (style) {
        result.style = style;
    }
    ...
    if (useFallback && fallback) {
        ...
        const { ultimate: fallbackUltimate } = generateFont(fallbackInfo, src, localFontPath, useFallback, 
        /* usePath = */ usePath && !path, append);
 ...
 */
export function getFontSubstitution(systemFontCache, idFactory, localFontPath, baseFontName, standardFontName, type) {
...
/*80--------------------------------------------------------------------------*/
//# sourceMappingURL=font_substitutions.js.map