...
export class FontExpotDataEx extends FontExpotData {
    differences;
    widths;
    cMap;
    defaultEncoding;
...
function createOS2Table(properties, charstrings, override) {
    ...
    let firstCharIndex = null, lastCharIndex = 0;
    if (charstrings) {
        let code;
...
export class Font extends FontExpotDataEx {
    ...
    constructor(name, file, properties) {
        ...
        if (!isSerifFont && !properties.isSimulatedFlags) {
            const baseName = name.replace(/[,_]/g, "-").split("-")[0], serifFonts = getSerifFonts();
    ...
    checkAndRepair(name, font, properties) {
        ...
        function readTrueTypeCollectionData(ttc, fontName) {
            ...
            if (fallbackData) {
                warn(`TrueType Collection does not contain "${fontName}" font, ` +
    ...
    charsToGlyphs(chars) {
        ...
        if (this.cMap) {
            ...
            const c = Object.create(null), ii = chars.length;
            let i = 0;
            while (i < ii) {
                ...
                const glyph = this.#charToGlyph(charcode, length === 1 && chars.charCodeAt(i - 1) === 0x20);
                glyphs.push(glyph);
            }
        }
        else {
