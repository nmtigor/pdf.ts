...
export function setMinMaxDimensions(node, style) {
    const parent = node[$getSubformParent]();
...
export function layoutNode(node, availableSpace) {
    let height;
    let width;
    let isBroken = false;
...
export function computeBbox(node, html, availableSpace) {
    let bbox;
    if (node.w !== "" && node.h !== "") {
...
export function layoutClass(node) {
    switch (node.layout) {
        case "position":
            return "xfaPosition";
        case "lr-tb":
...
export function toStyle(node, ...names) {
...
export function createWrapper(node, html) {
...
export function fixTextIndent(styles) {
...
export function setAccess(node, classNames) {
    switch (node.access) {
        case "nonInteractive":
...
export function isPrintOnly(node) {
    return (node.relevant.length > 0 &&
...
export function setFontFamily(xfaFont, node, fontFinder, style) {
    if (!fontFinder) {
