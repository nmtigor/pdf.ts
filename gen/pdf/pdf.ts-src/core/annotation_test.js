...
builtInCMapCache.set("UniJIS-UTF16-H", await CMapReaderFactory.fetch({ name: "UniJIS-UTF16-H" }));
builtInCMapCache.set("Adobe-Japan1-UCS2", await CMapReaderFactory.fetch({ name: "Adobe-Japan1-UCS2" }));
...
console.log("%c>>>>>>> test MarkupAnnotation >>>>>>>", `color:${css_1}`);
{
    ...
    console.log("it should parse IRT and set default RT when not defined...");
    {
        ...
        const { data } = (await AnnotationFactory.create(xref, replyRef, pdfManagerMock, idFactoryMock));
