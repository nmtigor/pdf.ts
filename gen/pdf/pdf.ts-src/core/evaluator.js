...
import { getNormalizedUnicodes, getUnicodeForGlyph, reverseIfRtl, } from "./unicode.js";
...
import { LocalColorSpaceCache, LocalGStateCache, LocalImageCache, LocalTilingPatternCache, } from "./image_utils.js";
...
export class PartialEvaluator {
    ...
    handleSetFont(resources, fontArgs, fontRef, operatorList, task, state, fallbackFontDict, cssFontInfo) {
        const fontName = fontArgs && fontArgs[0] instanceof Name ? fontArgs[0].name : undefined;
            ...
            switch (key) {
                ...
                case "Font":
                    ...
                    promise = promise.then(() => {
                        return this.handleSetFont(resources, undefined, value[0], operatorList, task, stateManager.state).then(loadedName => {
    ...
    loadFont(fontName, font, resources, fallbackFontDict, cssFontInfo) {
        const errorFont = async (font_y) => {
            return new TranslatedFont({
                loadedName: "g_font_error",
    ...
    parseShading({ shading, resources, localColorSpaceCache, localShadingPatternCache, }) {
        ...
        if (!id) {
            const shadingFill = Pattern.parseShading(shading, this.xref, resources, this.handler, this._pdfFunctionFactory, localColorSpaceCache);
    ...
    getOperatorList({ stream, task, resources, operatorList, initialState, fallbackFontDict, }) {
                    ...
                    case OPS.paintXObject:
                        ...
                        next(new Promise((resolveXObject, rejectXObject) => {
                            ...
                            if (type.name === "Form") {
                                stateManager.save();
                                self
                                    .buildFormXObject(resources, xobj, undefined, operatorList, task, stateManager.state.clone(), localColorSpaceCache)
                                    .then(function () {
    ...
    extractWidths(dict, descriptor, properties) {
        ...
        if (properties.composite) {
            ...
            if (properties.vertical) {
                const vmetrics_ = dict.getArray("DW2") ?? [880, -1000];
...
export class TranslatedFont {
    ...
    constructor({ loadedName, font, dict, evaluatorOptions }) {
        ...
        this.type3Dependencies = font.isType3Font ? new Set() : undefined;
    }
    send(handler) {
        if (this.sent) {
            return;
    ...
    fallback(handler) {
        ...
        PartialEvaluator.buildFontPaths(this.font, 
...
export class EvaluatorPreprocessor {
    ...
    nonProcessedArgs = [];
    #numInvalidPathOPS = 0;
    constructor(stream, xref, stateManager = new StateManager(new EvalState())) {
