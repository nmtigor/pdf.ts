...
async function writeSVG(svgElement, ctx) {
    ...
    const svg_xml = unescape(encodeURIComponent(new XMLSerializer().serializeToString(svgElement)));
...
class Rasterize {
    ...
    static async annotationLayer(ctx, viewport, outputScale, annotations, annotationCanvasMap, annotationStorage, fieldObjects, page, imageResourcesPath, renderForms = false) {
        try {
            ...
            const annotationImageMap = await convertCanvasesToImages(annotationCanvasMap, outputScale);
    ...
    static async highlightLayer(ctx, viewport, textContent) {
        try {
            ...
            const outlinerForOutline = new Outliner(boxes, 
            /* borderWidth = */ 0.0025, 
            /* innerMargin = */ 0.001);
...
class Snapshot_ extends HTMLVuu {
    static #bg = "#fff";
    static nextTaskBg() {
        ...
        this._cleanup().then(() => {
            ...
            if (prevFile) {
                if (task.file !== prevFile) {
                    this._nextPage(task, `The "${task.file}" file is identical to the previously used "${prevFile}" file.`);
                    return;
                }
            }
            else {
            ...
            try {
                    ...
                    for (const annotation of Object.values(task.annotationStorage)) {
                        ...
                        if (bitmapName) {
                            promise = promise.then(async (doc) => {
                                const response = await fetch(new URL(`./images/${bitmapName}`, window.location));
                ...
                promise.then(async (doc) => {
                    ...
                    if (task.optionalContent) {
                        const entries = Object.entries(task.optionalContent), optionalContentConfig = await task.optionalContentConfigPromise;
        ...
        if (!failure) {
            try {
                ...
                task.pdfDoc.getPage(task.pageNum).then((page) => {
                    ...
                    if (Math.floor(largestDimension * outputScale) >
                    ...
                    let renderAnnotations = false, renderForms = false, renderPrint = false, renderXfa = false, annotationCanvasMap, pageColors;
                    if (task.annotationStorage) {
                    ...
                    if (task.type === "text" || task.type === "highlight") {
                        ...
                        textLayerContext.clearRect(0, 0, textLayerCanvas.width, textLayerCanvas.height);
                        ...
                        initPromise = page
                            ...
                            return task.type === "text"
                                ? Rasterize.textLayer(textLayerContext, viewport, textContent)
                                : Rasterize.highlightLayer(textLayerContext, viewport, textContent);
                        });
                    }
                    else {
                        ...
                        if (renderAnnotations || renderForms || renderXfa) {
                            ...
                            annotationLayerContext.clearRect(0, 0, annotationLayerCanvas.width, annotationLayerCanvas.height);
                            ...
                            if (!renderXfa) {
                                ...
                                initPromise = page.getXfa().then((xfaHtml) => {
                                    return Rasterize.xfaLayer(annotationLayerContext, viewport, xfaHtml, task.fontRules, task.pdfDoc.annotationStorage, task.renderPrint);
                                });
                            }
                        }
                        else {
                    ...
                    initPromise
                        ...
                        return renderTask.promise.then(() => {
                            if (annotationCanvasMap) {
                                Rasterize.annotationLayer(annotationLayerContext, viewport, outputScale, data, annotationCanvasMap, task.pdfDoc.annotationStorage, task.fieldObjects, page, IMAGE_RESOURCES_PATH, renderForms).then(() => {
