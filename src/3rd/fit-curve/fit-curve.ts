 ...
 */
export function fitCurve<Arr extends number[] = [number, number]>(
  points: Point<Arr>[],
  maxError: number,
  progressCallback?: (_: _ProgressCallback<Arr>) => void,
): Curve<Arr>[] {
...
export function fitCubic<Arr extends number[] = [number, number]>(
  ...
  var bezCurve: Curve<Arr>, //Control points of fitted Bezier curve
    u: number[], //Parameter values for point
    ...
    centerVector,
    toCenterTangent: Vector<Arr>,
    fromCenterTangent, //Unit tangent vector(s) at splitPoint
    beziers: Curve<Arr>[], //Array of fitted Bezier curves if we need more than one curve
    dist,
    i;
  ...
  toCenterTangent = <Vector<Arr>> maths.normalize(centerVector);
...
function generateAndReport<Arr extends number[]>(
  points: Point<Arr>[],
  paramsOrig: number[],
  paramsPrime: number[],
  leftTangent: Vector<Arr>,
 ...
 * @param parameters Parameter values for region
 * @param leftTangent Unit tangent vector at start point
 * @param rightTangent Unit tangent vector at end point
 * @return Approximated Bezier curve: [first-point, control-point-1, control-point-2, second-point] where points are [x, y]
 */
function generateBezier<Arr extends number[]>(
  points: Point<Arr>[],
  parameters: number[],
  leftTangent: Vector<Arr>,
  rightTangent: Vector<Arr>,
  ...
  bezCurve = [firstPoint, undefined, undefined, lastPoint];
  ...
  if (alpha_l < epsilon || alpha_r < epsilon) {
    ...
    bezCurve[1] = <Point<Arr>> maths.addArrays(
      firstPoint,
      maths.mulItems(leftTangent, segLength / 3.0),
    );
    bezCurve[2] = <Point<Arr>> maths.addArrays(
      lastPoint,
 ...
 * @param point Digitized point
 * @param u Parameter value for "P"
 * @return New u
 */
function newtonRaphsonRootFind<Arr extends number[]>(
  bez: Curve<Arr>,
  point: Point<Arr>,
  u: number,
): number {
 ...
 */
function chordLengthParameterize<Arr extends number[]>(
  points: Point<Arr>[],
): number[] {
  var u: number[] = [], currU, prevU: number, prevP: Point<Arr>;
 ...
 *
 * @param points Array of digitized points
 * @param bez Fitted curve
 * @param parameters Parameterization of points
 * @return Maximum error (squared) and point of max error
 */
function computeMaxError<Arr extends number[]>(
  points: Point<Arr>[],
  bez: Curve<Arr>,
  parameters: number[],
): [number, number] {
...
var mapTtoRelativeDistances = function <Arr extends number[]>(
  bez: Curve<Arr>,
  B_parts: number,
): number[] {
  var B_t_curr;
...
function find_t<Arr extends number[]>(
  bez: Curve<Arr>,
  param: number,
  t_distMap: number[],
  B_parts: number,
  ...
  */
  var lenMax, lenMin, tMax, tMin, t: number;
...
export function createTangent<Arr extends number[] = [number, number]>(
  ...
  return <Vector<Arr>> maths.normalize(maths.subtract(pointA, pointB));
}
...
class maths {
  ...
  static zeros_Xx2x2<Arr extends number[]>(
    x: number,
  ): [number | Arr, number | Arr][] {
    const zs: [number | Arr, number | Arr][] = [];
    while (x--) zs.push([0, 0]);
    return zs;
  }

  //multiply = logAndRun(math.multiply);
  static mulItems<Arr extends number[]>(items: Arr, multiplier: number): Arr {
    return <Arr> items.map((x) => x * multiplier);
  }
  static mulMatrix<Arr extends number[]>(m1: Arr, m2: Arr): number {
  ...
  static subtract<Arr extends number[]>(arr1: Arr, arr2: Arr): Arr {
    return <Arr> arr1.map((x1, i) => x1 - arr2[i]);
  }

  //add = logAndRun(math.add);
  static addArrays<Arr extends number[]>(arr1: Arr, arr2: Arr): Arr {
    return <Arr> arr1.map((x1, i) => x1 + arr2[i]);
  }
  static addItems<Arr extends number[]>(items: Arr, addition: number): Arr {
    return <Arr> items.map((x) => x + addition);
  }

  //var sum = logAndRun(math.sum);
  static sum<Arr extends number[]>(items: Arr): number {
  ...
  static dot<Arr extends number[]>(m1: Arr, m2: Arr): number {
    return maths.mulMatrix(m1, m2);
  }

  //https://en.wikipedia.org/wiki/Norm_(mathematics)#Euclidean_norm
  //  var norm = logAndRun(math.norm);
  static vectorLen<Arr extends number[]>(v: Arr) {
    return Math.hypot(...v);
  }

  //math.divide = logAndRun(math.divide);
  static divItems<Arr extends number[]>(items: Arr, divisor: number) {
    return items.map((x) => x / divisor);
  }

  //var dotPow = logAndRun(math.dotPow);
  static squareItems<Arr extends number[]>(items: Arr): Arr {
    return <Arr> items.map((x) => x * x);
  }

  static normalize<Arr extends number[]>(v: Arr) {
...
class bezier {
  ...
  static q<Arr extends number[]>(ctrlPoly: Curve<Arr>, t: number): Arr {
    var tx = 1.0 - t;
    var pA = maths.mulItems(ctrlPoly[0], tx * tx * tx),
      pB = maths.mulItems(ctrlPoly[1], 3 * tx * tx * t),
      pC = maths.mulItems(ctrlPoly[2], 3 * tx * t * t),
      pD = maths.mulItems(ctrlPoly[3], t * t * t);
    return <Arr> maths.addArrays(
  ...
  static qprime<Arr extends number[]>(ctrlPoly: Curve<Arr>, t: number): Arr {
    var tx = 1.0 - t;
    ...
    return <Arr> maths.addArrays(maths.addArrays(pA, pB), pC);
  }

  //Evaluates cubic bezier second derivative at t, return point
  static qprimeprime<Arr extends number[]>(
    ctrlPoly: Curve<Arr>,
    t: number,
  ): Arr {
    return <Arr> maths.addArrays(
