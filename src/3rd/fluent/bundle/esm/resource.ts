...
export class FluentResource {
  ...
  constructor(source: string) {
    ...
    function consumeChar(char: string, errorClass?: SyntaxErrorConstructor) {
    ...
    // (was the match found?) or, if errorClass is passed, as an assertion.
    function consumeToken(re: RegExp, errorClass?: SyntaxErrorConstructor) {
      if (test(re)) {
        cursor = re.lastIndex;
        return true;
      }
    ...
    function parseMessage(id: string) {
      ...
      return { id, value: value!, attributes };
    }
    ...
    function parsePatternElements(
      elements: (string | Indent | Expression | null)[] = [],
      commonIndent: number,
    ): PatternElement[] {
      while (true) {
        if (test(RE_TEXT_RUN)) {
    ...
    function parsePlaceable(): Expression | null {
      consumeToken(TOKEN_BRACE_OPEN, SyntaxError);
      let selector = parseInlineExpression();
      if (consumeToken(TOKEN_BRACE_CLOSE)) {
        return selector as null;
      }
    ...
    function parseInlineExpression(): Expression | null {
      if (source[cursor] === "{") {
    ...
    function parseArguments() {
      let args: (Expression | NamedArgument)[] = [];
      while (true) {
