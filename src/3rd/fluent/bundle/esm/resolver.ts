...
import type { FluentValue } from "./types.ts";
import type {
  ComplexPattern,
  Expression,
  FunctionReference,
...
function match(scope: Scope, selector: FluentValue, key: FluentValue) {
  if (key === selector) {
    // Both are strings.
    return true;
  }
  ...
  if (selector instanceof FluentNumber && typeof key === "string") {
    let category = (scope.memoizeIntlObject(
      Intl.PluralRules,
      selector.opts,
    ) as Intl.PluralRules)
...
function getDefault(scope: Scope, variants: Variant[], star: number) {
  if (variants[star]) {
...
function getArguments(scope: Scope, args: (Expression | NamedArgument)[]) {
  const positional: FluentValue[] = [];
...
function resolveVariableReference(scope: Scope, { name }: VariableReference) {
  let arg;
  if (scope.params) {
 ...
 */
function resolvePattern(scope: Scope, value: Pattern) {
