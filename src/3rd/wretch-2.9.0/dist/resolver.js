...
export const resolver = (wretch) => {
    ...
    .then((response) => {
      if (!response.ok) {
        ...
        return response.text().then((body) => {
          ...
          err.message = body;
          if (
            config.errorType === "json" ||
            ((_a = response.headers.get("Content-Type")) === null ||
                  _a === void 0
                ? void 0
                : _a.split(";")[0]) === "application/json"
          ) {
            try {
  ...
  const catchersWrapper = (promise) => {
    return promise.catch((err) => {
      ...
      const catcher =
        ...
        (fetchErrorFlag && catchers.has(FETCH_ERROR) &&
  ...
  const bodyParser = (funName) => (cb) =>
    funName
      ...
      ? catchersWrapper(
        throwingPromise.then((_) => _ && _[funName]()).then((_) =>
          cb ? cb(_) : _
        ),
      )
  ...
  const enhancedResponseChain = addons.reduce((chain, addon) => ({
    ...chain,
    ...(typeof addon.resolver === "function"
      ? addon.resolver(chain)
      : addon.resolver),
  }), responseChain);
  return resolvers.reduce(
    (chain, r) => r(chain, wretch),
    enhancedResponseChain,
  );
};
