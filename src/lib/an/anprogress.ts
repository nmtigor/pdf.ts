...
import { id_t, Ratio } from "../alias.js";
import { Moo } from "../mv.js";
import { assert } from "../util/trace.js";
import { HTMLAnvuu } from "./anvuu.js";
import { createPromiseCap, PromiseCap } from "../promisecap.js";
/*81---------------------------------------------------------------------------*/
...
export class Anprogress
{
   ...
   */
  constructor( min_x:number, max_x:number, anvuu:HTMLAnvuu, anfun=new Anlinear() )
  {
    this.#anfun = anfun;
    this.#anvuu = anvuu;
   ...
   * @param { const } ratio
   */
  valOf( ratio:Ratio ) { return this.#min + this.#dur * ratio; }
  ...
  #tick = ( ts_x:number ) => {
    ...
    if( ts_x < this.ts_$! ) { this.#reqanfId = requestAnimationFrame(this.#tick); return; }
  ...
  play( src_x?:number, dur_x?:number, ts0_x?:number )
  {
    ...
    if( src1 < this.#min ) src1 = this.#min; else if( src1 > this.#max ) src1 = this.#max;
    if( tgt1 < this.#min ) tgt1 = this.#min; else if( tgt1 > this.#max ) tgt1 = this.#max;
    this.#srcing = src1;
    this.#tgting = tgt1;
  ...
  reversing() { this.diring = <1 | -1>-this.#diring; }
}
/*81---------------------------------------------------------------------------*/
