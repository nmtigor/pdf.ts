...
export class Util extends PDFObject<_SendUtilData> {
    ...
    return args[0].replaceAll(
      ...
      (match, nDecSep, cFlags, nWidth, nPrecision, cConvChar) => {
        ...
        const [thousandSep, decimalSep] =
          separators[<keyof typeof separators> nDecSep];
        ...
        if (thousandSep && intPart >= 1000) {
          ...
          intPart_ = buf.reverse().join(thousandSep);
        } else {
          intPart_ = intPart.toString();
        }

        let n = `${intPart_}${decPart}`;
        if (nWidth !== undefined) {
  ...
  scand(cFormat: CFormat, cDate: number | string | Date): Date | undefined {
    ...
    if (!this._scandCache.has(cFormat)) {
      ...
      const handlers: Record<string, _Handler> = {
        mmmm: {
          pattern: `(${months.join("|")})`,
          action: (value, data) => {
        ...
        mmm: {
          pattern: `(${
            months.map((month) => month.substring(0, 3)).join("|")
          })`,
          action: (value, data) => {
