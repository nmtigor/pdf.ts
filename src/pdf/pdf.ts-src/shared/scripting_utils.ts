...
{
  export function CMYK_G([ c, y, m, k ]:CYMK )
  {
    return <["G",number]>["G", 1 - Math.min(1, 0.3 * c + 0.59 * m + 0.11 * y + k)];
  }

  export function G_CMYK([ g ]:[number] )
  {
    return <["CMYK",...CYMK]>["CMYK", 0, 0, 0, 1 - g];
  }
  ...
  export function CMYK_RGB([ c, y, m, k ]:CYMK )
  {
    return <["RGB",...RGB]>[
      "RGB",
      1 - Math.min(1, c + k),
