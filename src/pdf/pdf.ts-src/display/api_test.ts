...
console.log("%c>>>>>>> test Page >>>>>>>",`color:${css_1}`);
{
  ...
  console.log(`${++i}: it gets annotations containing relative URLs (bug 766086)...`);
  {
    ...
    const defaultPromise = defaultLoadingTask.promise.then( 
      pdfDoc => pdfDoc.getPage(1).then( pdfPage => pdfPage.getAnnotations() )
    );
...
page 1 / 3`);
  ...
  console.log(`${++i}: it gets text content, with correct properties (issue 8276)...`);
  {
    ...
    const fontName = (<TextItem>items[0]).fontName;
...
sources, for full support with Dvips.`)
  ...
  console.log(`${++i}: it gets operator list, with ${"`annotationMode`"}-option...`);
  {
    ...
    console.assert( 
      opListAnnotDisable.fnArray.length < opListAnnotEnableForms.fnArray.length
    );
    console.assert( 
      opListAnnotEnableForms.fnArray.length < opListAnnotEnable.fnArray.length
    );
    console.assert( 
      opListAnnotEnable.fnArray.length < opListAnnotEnableStorage.fnArray.length
    );
  ...
  console.log(`${++i}: it cancels rendering of page...`);
  {
    ...
    try {
      ...
      console.assert( (<RenderingCancelledException>reason).message === "Rendering cancelled, page 1" );
      console.assert( (<RenderingCancelledException>reason).type === "canvas" );
    }
  ...
  console.log(`${++i}: it cleans up document resources during rendering of page...`);
  {
    ...
    try {
      ...
      console.assert( reason instanceof Error );
      console.assert( (<Error>reason).message ===
...
console.log("%c>>>>>>> test PDFDataRangeTransport >>>>>>>",`color:${css_1}`);
{
  ...
  console.log(`it should fetch document info and page, without range, using complete initialData...`);
  {
    ...
    const loadingTask = getDocument(<any>{
      disableRange: true,
      range: transport,
    });
    const pdfDocument = await loadingTask.promise;
