...
// #if !PRODUCTION || GENERIC
  ...
  const convertImgDataToPng = (function () {
    ...
    function writePngChunk( type:string, body:Uint8Array, data:Uint8Array, offset:number )
    {
      let p = offset;
    ...
    return function convertImgDataToPng(imgData:ImgData, forceDataSchema:boolean, isMask:boolean )
    {
      const kind =
  ...
  export class SVGGraphics
  {
    ...
    [ OPS.showText ]( glyphs:(Glyph|number|null)[] )
    {
      ...
      || fillStrokeMode === TextRenderingMode.FILL_STROKE
        ...
        {
          current.tspan!.setAttributeNS( null, "fill-opacity", <any>current.fillAlpha );
        }
      } 
