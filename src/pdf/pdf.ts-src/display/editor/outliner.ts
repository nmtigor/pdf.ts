...
import type { Box, Dot } from "../../alias.ts";
...
export class Outliner {
  ...
  #getOutlines(outlineVerticalEdges: Edge_[]) {
...
export class HighlightOutline extends Outline {
  ...
  serialize([blX, blY, trX, trY]: rect_t, _rotation: number): Outlines {
    ...
    for (const outline of this.#outlines) {
      const points = new Array<number>(outline.length);
...
export class FreeOutliner {
  ...
  toSVGPath() {
    ...
    const lastBottom = this.#last.subarray(16, 18);
    const [x, y, width, height] = this.#box;
    const [lastTopX, lastTopY, lastBottomX, lastBottomY] = this
      .#getLastCoords();
...
export class FreeHighlightOutline extends Outline {
  #box;
  #bbox: Box | undefined;
  #innerMargin;
  #isLTR;
    ...
    for (let i = 6, ii = outline.length; i < ii; i += 6) {
      if (isNaN(outline[i])) {
        ...
        const bbox = Util.bezierBoundingBox(
          lastX,
          lastY,
          ...outline.slice(i, i + 6) as unknown as TupleOf<number, 6>,
        );
