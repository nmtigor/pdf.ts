 ...
 *
 * @module pdf/pdf.ts-src/display/metadata_test.ts
 * @license Apache-2.0
 ******************************************************************************/
...
describe("metadata", () => {
  it("should handle valid metadata", () => {
    ...
    assert(metadata.has("dc:title"));
    assertFalse(metadata.has("dc:qux"));

    assertEquals(metadata.get("dc:title"), "Foo bar baz");
    assertEquals(metadata.get("dc:qux"), undefined);

    assertEquals(metadata.getAll(), { "dc:title": "Foo bar baz" });
  });

  it("should repair and handle invalid metadata", () => {
    const data = "<x:xmpmeta xmlns:x='adobe:ns:meta/'>" +
      "<rdf:RDF xmlns:rdf='http://www.w3.org/1999/02/22-rdf-syntax-ns#'>" +
      "<rdf:Description xmlns:dc='http://purl.org/dc/elements/1.1/'>" +
    ...
    assertEquals(metadata.get("dc:title"), "PDF&");
    assertEquals(metadata.get("dc:qux"), undefined);

    assertEquals(metadata.getAll(), { "dc:title": "PDF&" });
  });

  it("should repair and handle invalid metadata (bug 1424938)", () => {
    const data = "<x:xmpmeta xmlns:x='adobe:ns:meta/' " +
  ...
  it("should gracefully handle incomplete tags (issue 8884)", () => {
  ...
  it('should gracefully handle "junk" before the actual metadata (issue 10395)', () => {
    ...
    assertEquals(metadata.get("dc:title"), "");
    assertEquals(metadata.get("dc:qux"), undefined);
  ...
  it("should gracefully handle unbalanced end tags (issue 10410)", () => {
    const data = '<?xpacket begin="Ã¯Â»Â¿" id="W5M0MpCehiHzreSzNTczkc9d"?>' +
      '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">' +
      '<rdf:Description rdf:about="" ' +
      'xmlns:pdf="http://ns.adobe.com/pdf/1.3/">' +
  ...
  it("should not be vulnerable to the billion laughs attack", () => {
    ...
    assertEquals(metadata.get("dc:title"), "a&lol9;b");
    assertEquals(metadata.get("dc:qux"), undefined);

    assertEquals(metadata.getAll(), { "dc:title": "a&lol9;b" });
  });
});
/*80--------------------------------------------------------------------------*/
