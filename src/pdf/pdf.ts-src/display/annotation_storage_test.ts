...
console.log("%c>>>>>>> test get or default value >>>>>>>",`color:${css_1}`);
{
  ...
  {
    const annotationStorage = new AnnotationStorage();
    annotationStorage.setValue("123A", <any>{
      value: "hello world",
      hello: "world",
    });

    const result = annotationStorage.getValue("123A", <any>{
      value: "an other string",
      world: "hello",
    });
...
console.log("%c>>>>>>> test set value >>>>>>>",`color:${css_1}`);
{
  ...
  console.log("it should call onSetModified() if value is changed...");
  {
    ...
    console.assert( <any>called === true );
...
console.log(`%cpdf/pdf.ts-src/display/annotation_storage_test: ${(performance.now()-strttime).toFixed(2)} ms`,`color:${css_2}`);
