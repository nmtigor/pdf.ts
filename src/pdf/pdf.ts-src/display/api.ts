...
import type { C2D, int, TypedArray, uint } from "@fe-lib/alias.ts";
...
import { PDFFetchStream } from "./fetch_stream.ts";
...
import { TextLayer } from "./text_layer.ts";
...
export function getDocument(
  ...
  const transportParams: TransportParams_ = {
    disableFontFace,
    fontExtraProperties,
    ownerDocument,
    pdfBug,
...
export class PDFDocumentProxy {
  ...
  constructor(pdfInfo: PDFInfo, transport: WorkerTransport) {
    ...
    /*#static*/ if (PDFJSDev || TESTING) {
      ...
      Object.defineProperty(this, "getAnnotArray", {
        value: (pageIndex: number) => this._transport.getAnnotArray(pageIndex),
      });
    }
  }
  ...
  getOptionalContentConfig(
    { intent = "display" } = {} as GetOptionalContentConfigP_,
  ) {
...
export class PDFPageProxy {
  ...
  _intentStates = new Map<string, IntentState>();
  destroyed = false;
  ...
  } = {} as GetTextContentP): ReadableStream<TextContent> {
   ...
   */
  getTextContent(params = {} as GetTextContentP): Promise<TextContent> {
    if (this._transport._htmlForXfa) {
      // TODO: We need to revisit this once the XFA foreground patch lands and
      // only do this for non-foreground XFA.
      return this.getXfa().then((xfa) =>
        XfaText.textContent(xfa as XFAElObj | undefined)
      );
    }
...
export class PDFWorker {
  ...
  static get #mainThreadWorkerMessageHandler():
    | (typeof WorkerMessageHandler)
    | undefined {
    try {
      return (globalThis as any).pdfjsWorker?.WorkerMessageHandler;
    } catch {
      return undefined;
    }
  }
...
class WorkerTransport {
  ...
  constructor(
    ...
    /*#static*/ if (PDFJSDev || TESTING) {
      ...
      Object.defineProperty(this, "getXFADatasets", {
        value: () =>
          this.messageHandler.sendWithPromise("GetXFADatasets", undefined),
      });
      Object.defineProperty(this, "getXRefPrevValue", {
        value: () =>
          this.messageHandler.sendWithPromise("GetXRefPrevValue", undefined),
      });
      Object.defineProperty(this, "getStartXRefPos", {
        value: () =>
          this.messageHandler.sendWithPromise("GetStartXRefPos", undefined),
      });
      Object.defineProperty(this, "getAnnotArray", {
  ...
  setupMessageHandler() {
    ...
    messageHandler.on("commonobj", ([id, type, exportedData]) => {
      ...
      switch (type) {
        ...
        case "CopyLocalImage":
          ...
          for (const pageProxy of this.#pageCache.values()) {
            for (const [, data] of pageProxy.objs) {
              if ((data as ImgData)?.ref !== imageRef) {
                continue;
              }
              if (!(data as ImgData).dataLen) {
                return undefined;
              }
              this.commonObjs.resolve(id, structuredClone(data) as ImgData);
              return (data as ImgData).dataLen;
            }
          }
          break;
        case "FontPath":
        case "Image":
        case "Pattern":
          this.commonObjs.resolve(id, exportedData as Cmds | ImgData);
          break;
        default:
    ...
    messageHandler.on("obj", ([id, pageIndex, type, imageData]) => {
      ...
      if (pageProxy._intentStates.size === 0) {
        (imageData as ImgData)?.bitmap?.close(); // Release any `ImageBitmap` data.
        return;
      }
  ...
  getPage(pageNumber: unknown) {
    if (
      ...
      (pageNumber as int) <= 0 ||
      (pageNumber as int) > this.#numPages!
    ) {
      return Promise.reject(new Error("Invalid page request."));
    }

    const pageIndex = pageNumber as uint - 1,
    ...
    const promise = this.messageHandler
      ...
      .then((pageInfo) => {
        ...
        if (pageInfo.refStr) {
          this.#pageRefCache.set(pageInfo.refStr, pageNumber as uint);
        }
  ...
  getOptionalContentConfig(renderingIntent: RenderingIntentFlag) {
    return this.#cacheSimpleMethod("GetOptionalContentConfig").then(
      (data) =>
        new OptionalContentConfig(
          data as OptionalContentConfigData | undefined,
          renderingIntent,
        ),
    );
  }
...
interface Objs_<T> {
  ...
  data: T | typeof INITIAL_DATA | undefined;
  // resolved:boolean;
}
...
export class PDFObjects<T> {
  #objs: Record<string, Objs_<T | undefined> | undefined> = Object.create(null);
