...
export function recoverGlyphName(
  name: string,
 ...
 */
export function type1FontGlyphMapping(
  properties: FontProps,
  builtInEncoding: (number | string)[] | undefined,
  glyphNames: (string | number)[],
) {
  const charCodeToGlyphId: Record<number, string | number> = Object.create(
    null,
  );
...
export const getVerticalPresentationForm = getLookupTableFactory((t) => {
