...
export class Binder
{
  ...
  constructor( root:XFAObject )
  {
    ...
    this.root.form = this.form = <Subform>(<XFAObject>root.template)[$clone]();
  }
  ...
  #bindValue( formNode:ExclGroup | Field | Subform, data:XmlObject, picture:unknown )
  {
  ...
  #findDataByNameToConsume( name:string, isValue:boolean, 
    dataNode:XmlObject, global:boolean 
  ) {
  ...
  #setProperties( formNode:ExclGroup | Field | Subform, dataNode:XmlObject )
  {
    ...
    if( !formNode.hasOwnProperty("setProperty") ) return;

    for( const { ref, target, connection } of <SetProperty[]>formNode.setProperty.children )
    {
      if (connection) 
      {
  ...
  #bindItems( formNode:ExclGroup | Field | Subform, dataNode:XFAObject )
  {
    ...
    for( const { ref, labelRef, valueRef, connection } 
  ...
  #bindOccurrences( formNode:ExclGroup | Field | Subform, matches:XmlObject[], picture:unknown )
  {
  ...
  #createOccurrences( formNode:ExclGroup | Field | Subform )
  {
    ...
    if( !((<any>parent)[name] instanceof XFAObjectArray) ) return;
  ...
  #bindElement( formNode:ExclGroup | Field | Subform, dataNode:XmlObject )
  {
    ...
    for( const child of formNode[$getChildren]() )
    {
      ...
      if (this._mergeMode === undefined && child[$nodeName] === "subform") 
      {
        ...
        {
          this.#bindOccurrences( <ExclGroup | Field | Subform>child, [<XmlObject>dataChildren[0]], null );
        } 
        else if( this.emptyMerge )
        {
          ...
          const dataChild = ((<ExclGroup | Field | Subform>child)[$data] = new XmlObject(
            nsId,
            child.name || "root"
          ));
          dataNode[$appendChild](dataChild);
          this.#bindElement( <ExclGroup | Field | Subform>child, dataChild );
        }
        continue;
      }
      ...
      let match = null;
      if( (<ExclGroup | Field | Subform>child).bind )
      {
        switch( (<ExclGroup | Field | Subform>child).bind!.match )
        {
          case "none":
          ...
          case "global":
            ...
            break;
          case "dataRef":
            if( !(<ExclGroup | Field | Subform>child).bind!.ref )
            {
              warn(`XFA - ref is empty in node ${child[$nodeName]}.`);
              this.#setAndBind( <ExclGroup | Field | Subform>child, dataNode );
              continue;
            }
            ref = (<ExclGroup | Field | Subform>child).bind!.ref;
            break;
          default:
            break;
        }
        if( (<ExclGroup | Field | Subform>child).bind!.picture )
        {
          picture = (<ExclGroup | Field | Subform>child).bind!.picture![$content];
        }
      }

      const [min, max] = this.#getOccurInfo( <ExclGroup | Field | Subform>child );
      ...
      if( ref )
      {
        ...
        if( match === undefined ) 
        {
          ...
          if( this.#isConsumeData() )
          {
            ...
            match = (<(XmlObject | XFAAttribute)[]>match).filter(node => !node[$consumed]);
          }
          if( match.length > max )
          {
          ...
          {
            match.forEach( node => {
              (<XmlObject | XFAAttribute>node)[$consumed] = true;
            });
          }
        }
      } 
          ...
          if (!match) 
          {
            ...
            const nsId =
              dataNode[$namespaceId] === NS_DATASETS
                ? -1
                : dataNode[$namespaceId];
            match = (<ExclGroup | Field | Subform>child)[$data] = new XmlObject(nsId, child.name);
            if (this.emptyMerge) 
            {
              match[$consumed] = true;
            }
      ...
      if (match) 
      {
        this.#bindOccurrences( <ExclGroup | Field | Subform>child, match, picture );
      } 
      else if( min > 0 )
      {
        this.#setAndBind( <ExclGroup | Field | Subform>child, dataNode );
