...
console.log("%c>>>>>>> test getInheritableProperty() >>>>>>>",`color:${css_1}`);
{
  ...
  {
    console.assert(
      getInheritableProperty({ dict: <any>null, key: "foo" }) === undefined
    );
    console.assert(
      getInheritableProperty({ dict: <any>undefined, key: "foo" }) === undefined
    );
  }
  ...
  console.log("it handles dictionaries that do not contain the property...");
  {
    ...
    console.assert(
      getInheritableProperty({ dict: emptyDict, key: "foo" }) === undefined
    );
    ...
    console.assert(
      getInheritableProperty({ dict: filledDict, key: "foo" }) === undefined
    );
  }
  ...
  console.log("it fetches the property if it is not inherited...");
  {
    ...
    const dict = new Dict( <any>xref );

    // Regular values should be fetched.
    dict.set("foo", "bar");
    console.assert(
      getInheritableProperty({ dict, key: "foo" }) === "bar"
    );

    // Array value should be fetched (with references resolved).
    dict.set("baz", ["qux", ref]);
    console.assert( 
      (<string[]>getInheritableProperty({ dict, key: "baz", getArray: true }))
      .eq(["qux", "quux"])
    );
  }
  ...
  console.log("it fetches the property if it is inherited and present on one level...");
  {
    const ref = Ref.get(10, 0);
    const xref = new XRefMock([{ ref, data: "quux" }]);
    const firstDict = new Dict( <any>xref );
    const secondDict = new Dict( <any>xref );
    firstDict.set("Parent", secondDict);

    // Regular values should be fetched.
    secondDict.set("foo", "bar");
    console.assert(
      getInheritableProperty({ dict: firstDict, key: "foo" }) === "bar"
    );

    // Array value should be fetched (with references resolved).
    secondDict.set("baz", ["qux", ref]);
    console.assert(
      (<string[]>getInheritableProperty({ dict: firstDict, key: "baz", getArray: true }))
      .eq(["qux", "quux"])
    );
  }
  ...
  console.log("it fetches the property if it is inherited and present on multiple levels...");
  {
    ...
    console.assert(
      getInheritableProperty({ dict: firstDict, key: "foo" }) === "bar1"
    );
    console.assert(
      (<string[]>getInheritableProperty({
        dict: firstDict,
        key: "foo",
        getArray: false,
        stopWhenFound: false,
...
console.log("%c>>>>>>> test toRomanNumerals() >>>>>>>",`color:${css_1}`);
{
  console.log("it handles invalid arguments...");
  {
    ...
    {
      try {
        toRomanNumerals( <any>input );
        console.assert( !!0, "Should throw but not.");
      } catch(e) {
        console.assert( (<any>e).toString() ===
...
console.log("%c>>>>>>> test isWhiteSpace() >>>>>>>",`color:${css_1}`);
{
  ...
  console.log("it handles non-space characters...");
  {
    ...
    console.assert( isWhiteSpace(<any>null) === false );
    console.assert( isWhiteSpace(<any>undefined) === false );
  }
}
...
console.log("%c>>>>>>> test validateCSSFont() >>>>>>>",`color:${css_1}`);
{
  ...
  console.log("it check font weight...");
  {
    ...
    validateCSSFont(cssFontInfo);
    console.assert( <any>cssFontInfo.fontWeight === "400" );
  }
  ...
  console.log("it check italic angle...");
  {
    ...
    validateCSSFont(cssFontInfo);
    console.assert( <any>cssFontInfo.italicAngle == "14" );
    ...
    validateCSSFont(cssFontInfo);
    console.assert( <any>cssFontInfo.italicAngle == "2.718" );
  }
}
/*81---------------------------------------------------------------------------*/
