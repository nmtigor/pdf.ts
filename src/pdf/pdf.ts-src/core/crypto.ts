...
export class ARCFourCipher
{
  ...
  encryptBlock( data:Uint8Array | Uint8ClampedArray )
  {
    let a = this.a,
      b = this.b;
    const s = this.s;
...
namespace Ns_calculateSHA512
{
  ...
  export function hash( data:Uint8Array, offset:number, length:number, mode384=false )
  {
    // initial hash values
...
export function calculateSHA384( data:Uint8Array, offset:number, length:number ) 
{
...
class NullCipher
{
  decryptBlock( data:Uint8Array | Uint8ClampedArray ) { return data; }

  encrypt( data:Uint8Array ) { return data; }
}
/*81---------------------------------------------------------------------------*/

abstract class AESBaseCipher 
{
  protected _s = new Uint8Array([
  ...
  protected _cyclesOfRepetition!:number; // bits
  ...
  #decrypt = ( input:Uint8Array, key:Uint8Array ):Uint8Array =>
  {
    let t, u, v;
    ...
    for (let i = this._cyclesOfRepetition - 1; i >= 1; --i) {
      ...
      for (let j = 0; j < 16; j += 4) {
        ...
        const s1 = this.#mix[state[j + 1]];
        const s2 = this.#mix[state[j + 2]];
        const s3 = this.#mix[state[j + 3]];
        t =
          s0 ^
          (s1 >>> 8) ^
          (s1 << 24) ^
  ...
  #encrypt = ( input:Uint8Array, key:Uint8Array ):Uint8Array =>
  {
    const s = this._s;
    ...
    for (let i = 1; i < this._cyclesOfRepetition; i++) {
      ...
      for (let j = 0; j < 16; j += 4) {
        ...
        const s3 = state[j + 3];
        t = s0 ^ s1 ^ s2 ^ s3;
        state[j + 0] ^= t ^ this.#mixCol[s0 ^ s1];
        state[j + 1] ^= t ^ this.#mixCol[s1 ^ s2];
        state[j + 2] ^= t ^ this.#mixCol[s2 ^ s3];
        state[j + 3] ^= t ^ this.#mixCol[s3 ^ s0];
      }
      // AddRoundKey
      for (let j = 0, k = i * 16; j < 16; ++j, ++k) {
        state[j] ^= key[k];
  ...
  #decryptBlock2 = ( data:Uint8Array | Uint8ClampedArray, finalize:boolean ) => 
  {
    ...
    for (let i = 0; i < sourceLength; ++i) {
      ...
      const plain = this.#decrypt(buffer, this._key);
  ...
  decryptBlock = ( data:Uint8Array | Uint8ClampedArray, finalize:boolean, iv:Uint8Array | null=null 
  ...
  encrypt( data:Uint8Array, iv?:Uint8Array ):Uint8Array
  {
    ...
    for (let i = 0; i < sourceLength; ++i) {
      ...
      const cipher = this.#encrypt(buffer, this._key);
      iv = cipher;
      result.push(cipher);
      buffer = new Uint8Array(16);
      bufferLength = 0;
...
export class AES128Cipher extends AESBaseCipher 
{
  ...
  /** @override */
  protected _expandKey( cipherKey:ArrayLike<number> )
  {
    const b = 176;
    const s = this._s;
...
namespace NsPDF20
{
  function calculatePDF20Hash( password:Uint8Array, input:Uint8Array, userBytes:Uint8Array ) 
  {
...
{
  constructor(
    public StringCipherConstructor:(() => NullCipher) | (() => AES128Cipher) | (() => AES256Cipher),
    public StreamCipherConstructor:(() => NullCipher) | (() => AES128Cipher) | (() => AES256Cipher),
  ) {
  }
  ...
  createStream( stream:BaseStream, length:number ) 
  {
    ...
    return new DecryptStream(
      stream,
      length,
      function cipherTransformDecryptStream( data:Uint8Array | Uint8ClampedArray, finalize:boolean )
      {
  ...
  decryptString( s:string ) 
  {
    ...
    let data:Uint8Array | Uint8ClampedArray = stringToBytes(s);
...
namespace NsCipherTransformFactory
{
  ...
  function decodeUserPassword( 
    ...
    let cipher:ARCFourCipher, userPassword:Uint8Array;
    if (revision >= 3) {
  ...
  function buildObjectKey( num:number, gen:number, encryptionKey:Uint8Array | Uint8ClampedArray, isAes:boolean ) 
  {
  ...
  function buildCipherConstructor( cf:any, name:Name, num:number, gen:number, key:Uint8Array | Uint8ClampedArray ) 
  {
  ...
  export class CipherTransformFactory
  {
    ...
    constructor( dict:Dict, fileId:string, password?:string )
    {
      ...
      let keyLength = <number|undefined>dict.get("Length");
      if( !keyLength )
      {
      ...
      // prepare keys
      const ownerPassword = stringToBytes( <string>dict.get("O") ).subarray(0, 32);
      const userPassword = stringToBytes( <string>dict.get("U") ).subarray(0, 32);
      ...
      let encryptionKey:Uint8Array | Uint8ClampedArray | null;
      if( algorithm !== 5 )
      {
        ...
        const ownerValidationSalt = stringToBytes( <string>dict.get("O") ).subarray(
          32,
          40
        );
        const ownerKeySalt = stringToBytes( <string>dict.get("O") ).subarray(40, 48);
        const uBytes = stringToBytes( <string>dict.get("U") ).subarray(0, 48);
        const userValidationSalt = stringToBytes( <string>dict.get("U") ).subarray(
          32, 
          40
        );
        const userKeySalt = stringToBytes( <string>dict.get("U") ).subarray(40, 48);
      ...
      if( algorithm >= 4 )
      {
        ...
        this.cf = cf;
        this.stmf = <Name>dict.get("StmF") ?? identityName;
        this.strf = <Name>dict.get("StrF") ?? identityName;
        this.eff = <Name>dict.get("EFF") ?? this.stmf;
      }
    }
