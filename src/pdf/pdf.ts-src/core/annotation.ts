...
import { MOZCENTRAL, PDFJSDev, TESTING } from "@fe-src/global.ts";
import type {
  AnnotStorageRecord,
  AnnotStorageValue,
...
import type { AnnotActions } from "./core_utils.ts";
import {
  collectActions,
  escapeString,
...
import type { DefaultAppearanceData } from "./default_appearance.ts";
import {
...
export class AnnotationFactory {
  static createGlobals(
    pdfManager: BasePdfManager,
  ): Promise<AnnotationGlobals | undefined> {
    return Promise.all([
  ...
  static async create(
    ...
    const pageIndex = collectFields
      ? await this.#getPageIndex(xref, ref, annotationGlobals.pdfManager)
      : undefined;
  ...
  static _create(
    ...
    pageRef: Ref | undefined = undefined,
    ...
    const parameters: AnnotationCtorP_ = {
      xref,
      ref,
      dict,
      subtype,
  ...
  static generateImages(
    annotations: IterableIterator<AnnotStorageValue> | AnnotStorageValue[],
    xref: XRef,
    isOffscreenCanvasSupported: boolean | undefined,
  ): Map<string, Promise<AnnotImage>> | undefined {
  ...
  static async saveNewAnnotations(
    ...
    let baseFontRef;
    const dependencies: AnnotSaveData[] = [];
    const promises: Promise<AnnotSaveData>[] = [];
      ...
      switch (annotation.annotationType) {
        ...
        case AnnotationEditorType.STAMP:
          ...
          if (image?.imageStream) {
            ...
            image.imageStream = image.smaskStream = undefined;
          }
          promises.push(
...
function getPdfColorArray(color: Uint8ClampedArray) {
...
export type AnnotationData =
  & {
    ...
    vertices?: Float32Array | undefined; /* PolylineAnnotation */
    ...
    inkLists?: Float32Array[]; /* InkAnnotation */
...
export class Annotation {
   ...
   */
  mustBeViewed(annotationStorage?: AnnotStorageRecord, _renderForms?: boolean) {
  ...
  constructor(params: AnnotationCtorP_) {
    const { dict, xref, annotationGlobals } = params; // Table 164
    ...
    this.ref = params.ref instanceof Ref ? params.ref : undefined;
    ...
    if (annotationGlobals.structTreeRoot) {
      let structParent = dict.get("StructParent") as number;
      structParent = Number.isInteger(structParent) && structParent >= 0
        ? structParent
        : -1;
  ...
  setDefaultAppearance(params: AnnotationCtorP_) {
   ...
   */
  setLineEndings(lineEndings: [LineEnding_, LineEnding_]) {
    if (MOZCENTRAL) {
  ...
  setRotation(mk: Dict | undefined, dict: Dict) {
    this.rotation = 0;
    let angle = mk instanceof Dict
      ? mk.get("R") as number || 0
      : dict.get("Rotate") as number || 0;
  ...
  setBorderAndBackgroundColors(mk: Dict | undefined) {
    if (mk instanceof Dict) {
      this.borderColor = getRgbColor(mk.getArray("BC") as number[]);
      this.backgroundColor = getRgbColor(mk.getArray("BG") as number[]);
    } else {
  ...
  async save(
    ...
    task: WorkerTask,
    annotationStorage?: AnnotStorageRecord,
  ): Promise<AnnotSaveReturn | undefined> {
    return undefined;
  }
  ...
  async extractTextContent(
    ...
    const text: string[] = [];
    const buffer: string[] = [];
    let firstPosition: dot2d_t | undefined;
    const sink = {
      ...
      enqueue(chunk: MActionMap["GetTextContent"]["Sinkchunk"], size: number) {
        for (const item of chunk.items as TextItem[]) {
  ...
  reset() {
    /*#static*/ if (PDFJSDev || TESTING) {
      if (
        ...
        fail("The appearance stream should always be reset.");
      }
    }
  ...
  protected constructFieldName$(dict: Dict) {
    ...
    while (loopDict.has("Parent")) {
      ...
      if (loopDict.has("T")) {
        fieldName.unshift(stringToPDFString(loopDict.get("T") as string));
      }
    }
...
interface SetDefaultAppearanceP_ {
  ...
  fillAlpha?: number | undefined;
  blendMode?: string;
  pointsCallback: (buffer: string[], points: Float32Array) => rect_t;
}

interface CreateNewDictP_ {
  apRef?: Ref;
  ap?: StringStream | undefined;
  oldAnnotation?: Dict | undefined;
}
...
export class MarkupAnnotation extends Annotation {
  ...
  constructor(params: AnnotationCtorP_) {
    ...
    if (dict.has("IRT")) {
      ...
      this.data.replyType = rt instanceof Name
        ? rt.name as AnnotationReplyType
        : AnnotationReplyType.REPLY;
    }
    let popupRef: Obj | undefined;

    if (this.data.replyType === AnnotationReplyType.GROUP) {
      // Subordinate annotations in a group should inherit
      // the group attributes from the primary annotation.
      const parent = dict.get("IRT") as Dict;

      this.setTitle(parent.get("T"));
      this.data.titleObj = this._title;

      this.setContents(parent.get("Contents") as string | undefined);
      ...
      if (!parent.has("C")) {
        ...
        this.setColor(parent.getArray("C") as number[]);
    ...
    this.data.popupRef = popupRef instanceof Ref
      ? popupRef.toString()
      : undefined;

    if (dict.has("RC")) {
      this.data.richText = XFAFactory.getRichTextAsHtml(
        dict.get("RC") as string,
      );
    }
  }
  ...
  static async createNewAnnotation(
    ...
    params?: CreateNewAnnotationP_,
  ): Promise<AnnotSaveData> {
    let oldAnnotation: Dict | undefined;
    if (annotation.ref) {
      oldAnnotation = ((await xref.fetchIfRefAsync(annotation.ref)) as Dict)
        .clone();
    } else {
  ...
  static async createNewPrintAnnotation(
    ...
    const newAnnotation =
      new (this.prototype.constructor as Constructor<MarkupAnnotation>)({
        dict: annotationDict,
        xref,
        annotationGlobals,
...
export class WidgetAnnotation extends Annotation {
  ...
  mustBeViewedWhenEditing(isEditing: boolean, modifiedIds?: Set<string>) {
  ...
  override async getOperatorList(
    ...
    const stream = new StringStream(content as string);
    await evaluator.getOperatorList({
      stream,
      task,
  ...
  override async save(
    ...
    let appearance: string | { needAppearances: boolean } | undefined;
    if (!this._needAppearances) {
      appearance = await this.getAppearance$(
        evaluator,
        task,
    ...
    if (
      (appearance as { needAppearances: boolean } | undefined)?.needAppearances
    ) {
    ...
    dict.set(
      ...
      Array.isArray(value)
        ...
        : stringToAsciiOrUTF16BE(value as string),
    );
  ...
  protected async getAppearance$(
    ...
    if (
      rotation === undefined && value === undefined &&
    ...
    if (encodingError && this._isOffscreenCanvasSupported) {
      ...
      const newFont = resources.getRaw("Font") as Dict;

      if (this._fieldResources.mergedResources.has("Font")) {
        const oldFont = this._fieldResources.mergedResources.get(
          "Font",
        ) as Dict;
    ...
    if (alignment === 0 || alignment > 2) {
      ...
      return (
        `/Tx BMC q ${colors}BT ` +
        defaultAppearance +
        ` 1 0 0 1 ${numberToString(defaultHPadding)} ${
          numberToString(
            bottomPadding,
          )
  ...
  async _getAppearance(
    ...
    return this.getAppearance$(evaluator, task, intent, annotationStorage);
  }
  ...
  protected computeFontSize$(
    ...
    return [this._defaultAppearance, fontSize, height / numberOfLines] as [
      string,
      number,
      number,
    ];
  ...
  _renderText(
    ...
    return `${shiftStr} ${numberToString(vPadding)} Td (${
      escapeString(text)
    }) Tj`;
  }
...
class TextWidgetAnnotation extends WidgetAnnotation {
  constructor(params: AnnotationCtorP_) {
    ...
    if (
      !Number.isInteger(alignment) || alignment as number < 0 ||
      alignment as number > 2
    ) {
      alignment = undefined;
    }
    this.data.textAlignment = alignment as number | undefined;
  ...
  override _getCombAppearance(
    ...
    return (
      `/Tx BMC q ${colors}BT ` +
      defaultAppearance +
      ` 1 0 0 1 ${numberToString(hPadding)} ${
        numberToString(
          vPadding + descent,
        )
      } Tm ${renderedComb}` +
...
class ButtonWidgetAnnotation extends WidgetAnnotation {
  ...
  async _saveCheckbox(
    ...
    return [{ ref: this.ref, data: buffer.join(""), xfa }] as AnnotSaveReturn;
  }

  async _saveRadioButton(
    evaluator: PartialEvaluator,
    task: WorkerTask,
    annotationStorage?: AnnotStorageRecord,
  ) {
    ...
    const dict = evaluator.xref.fetchIfRef(this.ref) as Dict;
    if (!(dict instanceof Dict)) {
      return undefined;
    }
    ...
    const buffer: string[] = [];
    let parentData: string | undefined;

    if (value) {
      if (this.parent instanceof Ref) {
        const parent = evaluator.xref.fetch(this.parent) as Dict;
        parent.set("V", name);
  ...
  _getDefaultCheckedAppearance(
    ...
    if (type === "check") {
      ...
      fail(`_getDefaultCheckedAppearance - unsupported type: ${type}`);
    }
  ...
  _processCheckBox(params: AnnotationCtorP_) {
    ...
    if (this.data.defaultFieldValue === undefined) {
  ...
  _processPushButton(params: AnnotationCtorP_) {
    const { dict, annotationGlobals } = params;
...
class ChoiceWidgetAnnotation extends WidgetAnnotation {
  ...
  constructor(params: AnnotationCtorP_) {
    ...
    const options = getInheritableProperty({ dict, key: "Opt" }); // Table 246
  ...
  protected override async getAppearance$(
    ...
    if (this.data.combo) {
      return super.getAppearance$(evaluator, task, intent, annotationStorage);
    }
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      if (exportedValue!.includes(exportValue as string)) {
        valueIndices.push(i);
      }
    }
    ...
    if (!fontSize) {
      ...
      for (const { displayValue } of this.data.options!) {
        const width = this.getTextWidth$(displayValue as string, font);
        if (width > lineWidth) {
          lineWidth = width;
  ...
  constructor(params: AnnotationCtorP_) {
    ...
    if (this.data.hasAppearance) {
      ...
      this.data.name = dict.has("Name")
        ? (dict.get("Name") as Name).name
        : "Note";
    }

    if (dict.has("State")) {
      this.data.state = dict.get("State") as string | undefined;
      this.data.stateModel = dict.get("StateModel") as string | undefined;
    } else {
...
export class PopupAnnotation extends Annotation {
  constructor(params: AnnotationCtorP_) {
    ...
    let parentItem = dict.get("Parent") as Dict;
    if (!parentItem) {
    ...
    if (rt instanceof Name && rt.name === AnnotationReplyType.GROUP) {
      // Subordinate annotations in a group should inherit
      // the group attributes from the primary annotation.
      parentItem = parentItem.get("IRT") as Dict;
    }
    ...
    if (!this.viewable) {
      const parentFlags = parentItem.get("F") as AnnotationFlag;
    ...
    this.setContents(parentItem.get("Contents") as string);
    this.data.contentsObj = this._contents;

    if (parentItem.has("RC")) {
      this.data.richText = XFAFactory.getRichTextAsHtml(
        parentItem.get("RC") as string,
      );
    }

    this.data.open = !!dict.get("Open") as boolean | undefined;
  }
}
...
class FreeTextAnnotation extends MarkupAnnotation {
  ...
  static override createNewDict(
    annotation: AnnotStorageValue,
    xref: XRef,
    { apRef, ap, oldAnnotation }: CreateNewDictP_,
  ) {
    ...
    freetext.set("DA", da);
    freetext.set("Contents", stringToAsciiOrUTF16BE(value as string));
    freetext.set("F", 4);
  ...
  static override async createNewAppearanceStream(
    ...
    let firstPoint: dot2d_t, clipBox: rect_t, matrix: rect_t;
    switch (rotation) {
      case 0:
        matrix = [1, 0, 0, 1];
...
class LineAnnotation extends MarkupAnnotation {
  constructor(params: AnnotationCtorP_) {
    ...
    /*#static*/ if (PDFJSDev || !MOZCENTRAL) {
      this.setLineEndings(dict.getArray("LE") as [LineEnding_, LineEnding_]);
    ...
    if (!this.appearance) {
      ...
      const strokeColor = this.color
        ? getPdfColorArray(this.color) as AColor
        : [0, 0, 0] as AColor;
      const strokeAlpha = dict.get("CA") as number | undefined;

      const interiorColor = getRgbColor(dict.getArray("IC") as number[]);
      ...
      const fillColor = interiorColor
        ? getPdfColorArray(interiorColor) as AColor
        : undefined;
  ...
  constructor(params: AnnotationCtorP_) {
    ...
    /*#static*/ if (PDFJSDev || !MOZCENTRAL) {
      if (!(this instanceof PolygonAnnotation)) {
        ...
        this.setLineEndings(<[LineEnding_, LineEnding_]> dict.getArray("LE"));
        this.data.lineEndings = this.lineEndings;
      }
    }
...
class InkAnnotation extends MarkupAnnotation {
  constructor(params: AnnotationCtorP_) {
    ...
    const rawInkLists = dict.getArray("InkList") as (number | Ref)[][];
    ...
    if (quadPoints) {
      ...
      if (!this.appearance || !resources?.has("ExtGState")) {
        ...
        const fillColor = this.color
          ? getPdfColorArray(this.color) as AColor
          : [1, 1, 0] as AColor;
        const fillAlpha = dict.get("CA") as number | undefined;
...
class StampAnnotation extends MarkupAnnotation {
  ...
  static async createImage(
    ...
    const ctx = canvas.getContext("2d", { alpha: true }) as OC2D;
    ...
    let smaskStream: Stream | undefined;
    if (hasAlpha) {
