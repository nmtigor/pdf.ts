 ...
 *
 * @module pdf/pdf.ts-src/core/object_loader.ts
 * @license Apache-2.0
 ******************************************************************************/
...
import { ChunkedStream, type ChunkRange } from "./chunked_stream.ts";
import { MissingDataException } from "./core_utils.ts";
import { Dict, type Obj, Ref, RefSet } from "./primitives.ts";
...
export class ObjectLoader {
  ...
  async #walk(nodesToVisit: Obj[]): Promise<ChunkedStream | undefined> {
        ...
        if (baseStreams) {
          ...
          for (const stream of baseStreams) {
            ...
            pendingRequests.push({
              begin: (<ChunkedStream> stream).start,
              end: (<ChunkedStream> stream).end,
            });
          }
    ...
    if (pendingRequests.length) {
      await (<ChunkedStream> this.xref.stream).manager.requestRanges(
        pendingRequests,
      );
