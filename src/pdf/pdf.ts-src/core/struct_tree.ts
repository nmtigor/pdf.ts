...
export class StructTreeRoot
{
  ...
  readRoleMap()
  {
    ...
    if( !(roleMapDict instanceof Dict) ) return;

    roleMapDict.forEach((key, value) => {
      if( !(value instanceof Name) ) return;
...
export class StructTreePage
{
  ...
  addTopLevelNode( dict:Dict, element:StructElementNode )
  {
    ...
    {
      if( obj.objId !== dict.objId ) return false;
  ...
  get serializable()
  {
    function nodeToSerializable( node:StructElementNode, parent:StructTree, level=0 )
    {
      ...
      const obj:StructTree = Object.create(null);
      obj.role = node.role;
      obj.children = [];
    ...
    const root:StructTree = Object.create(null);
    root.children = [];
    root.role = "Root";
