...
import type { CharCodeOut, CMap } from "./cmap.ts";
import { IdentityCMap } from "./cmap.ts";
...
import type { OTTable, OTTag } from "./opentype_file_builder.ts";
import { OpenTypeFileBuilder, VALID_TABLES } from "./opentype_file_builder.ts";
...
function amendFallbackToUnicode(properties: FontProps) {
  ...
  for (const charCode in properties.fallbackToUnicode) {
    if (
      (properties.toUnicode as ToUnicodeMap | IdentityToUnicodeMap).has(
        +charCode,
      )
    ) {
...
function getFontFileType(
  file: BaseStream,
  { type, subtype, composite }: FontfileType,
) {
  ...
  return [fileType, fileSubtype] as const;
}

function applyStandardFontGlyphMap(
  map: number[],
...
function buildToFontChar(
  encoding: string[],
  glyphsUnicodeMap: Record<string, number>,
  differences?: string[],
) {
  const toFontChar = [];
  let unicode;
...
function adjustMapping(
  charCodeToGlyphId: Record<string | number, number>,
  hasGlyph: (glyphId: number) => boolean,
...
function createCmapTable(
  ...
  for (i = ranges.length - 1; i >= 0; --i) {
    if ((ranges[i][0] as number) <= 0xffff) {
      break;
    }
  }
  const bmpLength = i + 1;

  if ((ranges[i][0] as number) < 0xffff && ranges[i][1] === 0xffff) {
    ranges[i][1] = 0xfffe;
  }
  ...
  if (numTables > 1) {
    ...
    for (i = 0, ii = ranges.length; i < ii; i++) {
      ...
      format31012 += string32(start) + // startCharCode
        string32(range[1] as number) + // endCharCode
        string32(code); // startGlyphID
    }
...
export class Font extends FontExpotDataEx {
  ...
  constructor(name: string, file: Stream | undefined, properties: FontProps) {
    ...
    this.toUnicode = properties.toUnicode as
  ...
  fallbackToSystemFont(properties: FontProps) {
    ...
    this.remeasure = (!isStandardFont || isNarrow) &&
      Object.keys(<Record<number, number>> this.widths).length > 0;
    if (
  ...
  checkAndRepair(name: string, font: BaseStream, properties: FontProps) {
    ...
    function readCmapTable(
      ...
      let potentialTable: PotentialTable_ | undefined;
      let canBreak = false;
    ...
    function sanitizeTTProgram(table: OTTable, ttContext: TTContext_) {
      let data = table.data;
      let i = 0,
        j,
        n,
    ...
    function checkInvalidFunctions(
      ttContext: TTContext_,
    ...
    function sanitizeTTPrograms(
      fpgm: OTTable | undefined,
      prep: OTTable | undefined,
      cvt: OTTable | undefined,
      maxFunctionDefs: number,
    ) {
      const ttContext: TTContext_ = {
        functionsDefined: [],
        functionsUsed: [],
    ...
    if (!isTrueType) {
      const isComposite = properties.composite &&
        (properties.cidToGidMap?.length as any > 0 ||
    ...
    if (!isTrueType) {
      try {
        ...
        tables["CFF "]!.data = compiler.compile() as any;
      } catch {
  ...
  convert(fontName: string, font: CFFFont | Type1Font, properties: FontProps) {
    ...
    let toUnicodeExtraMap: Map<number, number>;
  ...
  #charToGlyph(charcode: number, isSpace = false) {
    ...
    const vmetric = this.vmetrics?.[widthCode as number];
  ...
  encodeString(str: string) {
    ...
    const getCharCode = this.toUnicode instanceof IdentityToUnicodeMap
