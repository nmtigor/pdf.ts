...
export abstract class FontExpotData
{
  name!:string;
...
const EXPORT_DATA_PROPERTIES:readonly (keyof FontExpotData)[] = [
  "ascent",
  "bbox",
  "black",
  "bold",
...
export abstract class FontExpotDataEx extends FontExpotData
{
...
const EXPORT_DATA_EXTRA_PROPERTIES:readonly (keyof FontExpotDataEx)[] = [
  "cMap",
  "defaultEncoding",
  "differences",
  "isMonospace",
...
// type PropName = (typeof EXPORT_DATA_PROPERTIES)[number] | (typeof EXPORT_DATA_EXTRA_PROPERTIES)[number]

function adjustWidths( properties:FontProps ) 
{
...
function adjustToUnicode( properties:FontProps, builtInEncoding?:string[] )
{
  ...
  for( const charCode in builtInEncoding )
  {
    ...
    {
      if( (<ToUnicodeMap | IdentityToUnicodeMap>properties.toUnicode).has(+charCode) ) 
      {
    ...
    const glyphName = builtInEncoding[<any>charCode];
    const unicode = getUnicodeForGlyph(glyphName, glyphsUnicodeMap);
    if (unicode !== -1) {
      toUnicode[<any>charCode] = String.fromCharCode(unicode);
    }
  }
...
{
  if( !properties.fallbackToUnicode ) return;
...
export class Glyph
{
  ...
  constructor(
    ...
    public accent:Accent | undefined,
    public width:number | undefined,
    public vmetric:VMetric | undefined,
    public operatorListId:number | undefined,
...
function writeSignedInt16( bytes:Uint8Array | Uint8ClampedArray, index:number, value:number ) 
{
  bytes[index + 1] = value;
  bytes[index] = value >>> 8;
}
...
function isTrueTypeFile( file:BaseStream ) 
{
...
function isTrueTypeCollectionFile( file:BaseStream ) 
{
  const header = file.peekBytes(4);
  return bytesToString(header) === "ttcf";
}

function isOpenTypeFile( file:BaseStream ) 
{
  const header = file.peekBytes(4);
  return bytesToString(header) === "OTTO";
}

function isType1File( file:BaseStream ) 
{
 ...
 */
function isCFFFile( file:BaseStream ) 
{
  const header = file.peekBytes(4);
  if (
...
function getFontFileType( file:BaseStream, { type, subtype, composite }:FontfileType )
{
  ...
  return <const>[fileType, fileSubtype];
}

function applyStandardFontGlyphMap( map:number[], 
  glyphMap:Record<string | number, number>
) {
...
function buildToFontChar( encoding:string[], glyphsUnicodeMap:Record<string,number>, differences?:string[] )
{
  const toFontChar = [];
  let unicode;
 ...
 */
function adjustMapping( charCodeToGlyphId:Record<string|number,number>, 
  hasGlyph:( glyphId:number ) => boolean, newGlyphZeroId:number 
):AdjustMappingReturn {
...
function getRanges( glyphs:Record<number,number>, numGlyphs:number ) 
{
  ...
  for (const charCode in glyphs) 
  {
    ...
    if (glyphs[charCode] >= numGlyphs) continue;
...
function createCmapTable( glyphs:Record<number,number>, numGlyphs:number ) 
{
...
function validateOS2Table( os2:OTTable, file:BaseStream )
{
...
function createOS2Table( properties:FontProps, charstrings:Record<number,number>, override?:MetricsOverride )
{
  override = override || {
    unitsPerEm: 0,
    yMax: 0,
...
function createPostTable( properties:FontProps ) 
{
...
export class Font extends FontExpotDataEx
{
  ...
  _charsCache:Record<string, Glyph[]> = Object.create(null);
  _glyphCache:Glyph[] = Object.create(null);
  ...
  constructor( name:string, file:BaseStream | undefined, properties:FontProps )
  {
    ...
    this.toUnicode = <IdentityToUnicodeMap | ToUnicodeMap | undefined>properties.toUnicode;

    if( properties.type === "Type3" )
    {
      for( let charCode = 0; charCode < 256; charCode++ )
      {
        this.toFontChar[charCode] = <any>(
    ...
    try {
      switch( type ) 
      {
        ...
        case "CIDFontType2":
          ...
          // Repair the TrueType file. It can be damaged in the point of
  ...
  exportData( extraProperties=false )
  {
    ...
    const data:FontExpotDataEx = Object.create(null);
    let property, value;
    for( property of exportDataProperties )
    {
      ...
      if (value !== undefined) {
        (<any>data)[property] = value;
      }
    }
    return data;
  }

  fallbackToSystemFont( properties:FontProps )
  {
    ...
    this.remeasure =
      (!isStandardFont || isNarrow) && Object.keys(<Record<number,number>>this.widths).length > 0;
  ...
  {
    function readTables( file:BaseStream, numTables:number ) 
    {
      const tables:Record<OTTag, OTTable | undefined> = Object.create(null);
      tables["OS/2"] = undefined;
      tables.cmap = undefined;
      tables.head = undefined;
      tables.hhea = undefined;
      tables.hmtx = undefined;
      tables.maxp = undefined;
      tables.name = undefined;
      tables.post = undefined;

      for( let i = 0; i < numTables; i++ )
      {
        const table = readTableEntry(file);
        if( !VALID_TABLES.includes(<any>table.tag) )
        {
        ...
        tables[ <OTTag>table.tag ] = <OTTable>table;
      }
      return tables;
    }

    function readTableEntry( file:BaseStream )
    {
    ...
    function readOpenTypeHeader( ttf:BaseStream ) 
    {
      return {
    ...
    function readTrueTypeCollectionHeader( ttc:BaseStream )
    {
    ...
    function readTrueTypeCollectionData( ttc:BaseStream, fontName:string ) 
    {
     ...
     * PDF spec
     */
    function readCmapTable( cmap:OTTable | undefined, file:BaseStream, isSymbolicFont:boolean, hasEncoding?:boolean )
    {
      if( !cmap )
      {
    ...
    function sanitizeMetrics(
      file:BaseStream,
      header:OTTable | undefined,
      metrics:OTTable | undefined,
      headTable:OTTable | undefined,
      numGlyphs:number,
    ...
    function sanitizeHead( head:OTTable, numGlyphs:number, locaLength:number ) 
    {
    ...
    function readPostScriptTable( post:OTTable, propertiesObj:FontProps, maxpNumGlyphs:number ) 
    {
    ...
    {
      const start = ((<any>font).start ? (<any>font).start : 0) + nameTable.offset;
      font.pos = start;
    ...
    function foldTTTable( table:OTTable, content:(Uint8Array|Uint8ClampedArray)[] )
    {
      if( content.length > 1 )
      {
    ...
    function sanitizeTTPrograms( fpgm:OTTable | undefined, prep:OTTable | undefined, cvt:OTTable | undefined, maxFunctionDefs:number ) 
    {
    ...
    if( isTrueTypeCollectionFile(font) )
    {
      ...
      header = readOpenTypeHeader( <Stream>font );
      tables = readTables( <Stream>font, header.numTables );
    }
    let cff, cffFile;
    ...
    if( !isTrueType )
    {
      ...
      delete (<any>tables).glyf;
      delete (<any>tables).loca;
      delete (<any>tables).fpgm;
      delete (<any>tables).prep;
      delete (<any>tables)["cvt "];
    ...
    const metricsOverride:MetricsOverride = {
    ...
    if( properties.composite )
    {
      ...
      if( properties.glyphNames
        ...
        for (let i = 0; i < 256; ++i) 
        {
          ...
          const glyphName = this.differences![i] || baseEncoding[i];          // glyphName = this.differences![i] || baseEncoding[i];
          if (!glyphName) 
          {
            continue;
          }
    ...
    if( !properties.cssFontInfo )
    {
      ...
      const newMapping = adjustMapping(
        <Record<number,number>>charCodeToGlyphId,
        hasGlyph,
        glyphZeroId
      );
      ...
      tables.cmap = <OTTable>{
        tag: "cmap",
        data: <any>createCmapTable( newMapping.charCodeToGlyphId, numGlyphsOut ),
      };
    ...
    if (!isTrueType) {
      try {
        ...
        tables["CFF "]!.data = <any>compiler.compile();
      } catch (e) {
    ...
    {
      builder.addTable( <OTTag>tableTag, tables[<OTTag>tableTag]!.data );
    }
    return builder.toArray();
  }

  convert( fontName:string, font:CFFFont | Type1Font, properties:FontProps ) 
  {
    ...
    let newMapping:AdjustMappingReturn | undefined;
    ...
    function getCharCodes( charCodeToGlyphId:Record<string|number,number>, glyphId:number )
    {
    ...
    function createCharCode( charCodeToGlyphId:Record<number,number>, glyphId:number ) 
    {
      for( const charCode in charCodeToGlyphId )
      {
        if (glyphId === charCodeToGlyphId[charCode]) 
    ...
    if( SEAC_ANALYSIS_ENABLED && seacs && seacs.length )
    {
      ...
      const seacMap:Seac[] = Object.create(null);
      for( const glyphId_s in seacs )
      {
        const glyphId = +glyphId_s | 0;
        const seac = seacs[glyphId];
        const baseGlyphName = StandardEncoding[seac[2]];
        const accentGlyphName = StandardEncoding[seac[3]];
        const baseGlyphId = charset.indexOf(baseGlyphName);
        const accentGlyphId = charset.indexOf(accentGlyphName);
        if( baseGlyphId < 0 || accentGlyphId < 0 ) continue;
    ...
    builder.addTable(
      ...
      (function fontFieldsHmtx() {
        const charstrings = (<any>font).charstrings;
        const cffWidths = (<any>font).cff ? (<any>font).cff.widths : null;
  ...
  encodeString( str:string )
  {
    ...
    const getCharCode =
      this.toUnicode instanceof IdentityToUnicodeMap
        ? ( unicode:number ) => this.toUnicode!.charCodeOf(unicode)
        : ( unicode:number ) => this.toUnicode!.charCodeOf(String.fromCodePoint(unicode));
...
export class ErrorFont extends FontExpotData
{
  error;
  override loadedName = "g_font_error";
  override missingFile = true;
  ...
  getCharPositions( chars:string ) { return <[number,number][]>[]; }
