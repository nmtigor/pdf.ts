...
import { CMap, CMapFactory, IdentityCMap, type CharCodeOut } from "./cmap.js";
...
console.log("%c>>>>>>> test cmap >>>>>>>",`color:${css_1}`);
{
  ...
  let fetchBuiltInCMap = function( name:string ) 
  {
  ...
  console.log("it parses beginbfchar...");
  {
    ...
    const cmap = await CMapFactory.create(<any>{ encoding: stream });
  ...
  console.log("it attempts to load a non-existent built-in CMap...");
  {
    try {
      ...
      console.assert( (<Error>reason).message === "Unknown CMap name: null" );
    }
  }
  ...
  console.log("it attempts to load a built-in CMap without the necessary API parameters...");
  {
    ...
    {
      const CMapReaderFactory = new DOMCMapReaderFactory(<any>{});
  ...
  fetchBuiltInCMap = <any>undefined;
}
/*81---------------------------------------------------------------------------*/
