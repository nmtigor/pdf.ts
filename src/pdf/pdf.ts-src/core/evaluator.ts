...
function normalizeBlendMode( value:BMValue, parsingArray=false ):string | undefined
{
  ...
  {
    if( parsingArray ) return undefined;
  ...
  {
    case "Normal":
    case "Compatible": return "source-over";
...
class TimeSlotManager 
{
  ...
  check() 
  {
    if( ++this.checked < TimeSlotManager.CHECK_TIME_EVERY ) return false;
...
interface BuildTextContentItemParms
{
  chars:string;
  extraSpacing:number;
}
...
export class PartialEvaluator 
{
  ...
  {
    if( !(resources instanceof Dict) ) return false;
    ...
    while (nodes.length) 
    {
      ...
      if (graphicStates instanceof Dict) 
      {
        for (let graphicState of graphicStates.getRawValues()) 
        {
          ...
          if (!(graphicState instanceof Dict)) continue;
      ...
      if (!(xObjects instanceof Dict)) continue;
      ...
      for (let xObject of xObjects.getRawValues()) 
      {
        ...
        if( !(xObject instanceof BaseStream) ) continue;
        ...
        if( !(xResources instanceof Dict) ) continue;
  ...
  async fetchStandardFontData( name:string )
  {
    ...
    if( cachedData ) return new Stream( cachedData );

    // The symbol fonts are not consistent across platforms, always load the
    // standard font data for them.
    if( this.options.useSystemFonts 
     && name !== "Symbol" 
     && name !== "ZapfDingbats"
    ) {
      return undefined;
  ...
  {
    const transfers = imgData ? [imgData.data!.buffer] : undefined;
  ...
  handleSMask(
    ...
    {
      const transferFn = this._pdfFunctionFactory.create( <BaseStream|Dict>transferObj );
  ...
  handleTilingType(
    ...
    return this.getOperatorList({
      ...
      .then( () => {
        ...
        {
          localTilingPatternCache.set(/* name = */ undefined, patternDict.objId, {
            operatorListIR,
            dict: patternDict,
          });
        }
  ...
  handleSetFont( 
    ...
    const fontName =
      fontArgs && fontArgs[0] instanceof Name ? fontArgs[0].name : undefined;
    ...
    return this.loadFont( 
      ...
      .then( translated => {
        if( !translated.font.isType3Font ) return translated;
  ...
  loadFont( 
    ...
    const errorFont = async ( font_y:FontDict | null ) => {
      return new TranslatedFont({
        loadedName: "g_font_error",
  ...
  buildPath( operatorList:OperatorList, fn:OPS, args:OpArgs, parsingText=false )
  {
  ...
  parseColorSpace({ 
    ...
    return ColorSpace.parseAsync({
      ...
      if (reason instanceof AbortException) return undefined;
  ...
  handleColorN(
    ...
    if( patternName instanceof Name )
    {
      ...
      {
        try {
          const color = cs.base ? cs.base.getRgb(<number[]>args,0) : undefined;
          const tilingPatternIR = getTilingPatternIR(
      ...
      const pattern = <BaseStream | Dict | undefined>this.xref.fetchIfRef( rawPattern! );
      if( pattern )
      {
  ...
  async parseMarkedContentProps( contentProperties:Dict | Name, resources:Dict
  ...
  getOperatorList({
    ...
    return new Promise<void>( function promiseBody( resolve, reject )
    {
      ...
      while( !(stop = timeSlotManager.check()) )
      {
        ...
        let args = <OpArgs | Uint8ClampedArray | [Glyph[]]>operation.args;
        let fn = operation.fn;
        ...
        switch( fn | 0 ) 
        {
          case OPS.paintXObject:
            ...
            if( isValidName )
            {
              ...
              if( localImage )
              {
                operatorList.addOp( (<any>localImage).fn, (<any>localImage).args );
                args = null;
                continue;
              }
            }
          ...
          case OPS.endInlineImage:
            ...
            if( cacheKey )
            {
             ...
             if( localImage )
              {
                operatorList.addOp( 
                  (<LI_CData>localImage).fn, 
                  (<LI_CData>localImage).args
                );
                args = null;
                continue;
              }
          ...
          case OPS.nextLineSetSpacingShowText:
            ...
            operatorList.addOp(OPS.nextLine);
            operatorList.addOp(OPS.setWordSpacing, [(<unknown[]>args).shift()!]);
            operatorList.addOp(OPS.setCharSpacing, [(<unknown[]>args).shift()!]);
            args![0] = self.handleText( <string>args![0], stateManager.state );
            fn = OPS.showText;
            break;
          ...
          case OPS.shadingFill:
            ...
            const shading = <Dict | BaseStream>shadingRes.get( (<Name>args![0]).name );
            if (!shading) 
            {
          ...
          case OPS.closePath:
          case OPS.rectangle:
            self.buildPath(operatorList, fn, <OpArgs>args, parsingText);
            continue;
          case OPS.markPoint:
          ...
          case OPS.beginMarkedContentProps:
            ...
            {
              warn(`Expected name for beginMarkedContentProps arg0=${<any>args![0]}`);
              continue;
            }
            if( args![0].name === "OC" )
            {
          ...
          default:
            ...
            if( args !== null )
            {
              ...
              {
                if( args[i] instanceof Dict ) break;
              }
              if( i < ii )
              {
  ...
  }:GetTextContentParms ) 
  {
    ...
    const textContentItem:TextContentItem = {
      initialized: false,
      str: [],
      totalWidth: 0,
      totalHeight: 0,
    ...
    {
      if( textContentItem.initialized ) return textContentItem;
    ...
    {
      if( !textContentItem.initialized ) return;
      ...
      if( scaleFactor === textContentItem.textAdvanceScale ) return;
    ...
    function runBidiTransform( textChunk:TextContentItem ):BidiTextContentItem
    {
    ...
    function compareWithLastPosition()
    {
      ...
      if( lastPosX === posX && lastPosY === posY ) return;
    ...
    function buildTextContentItem({ chars, extraSpacing }:BuildTextContentItemParms )
    {
      const font = textState.font!;
      if( !chars )
      {
      ...
      for( let i = 0, ii = glyphs.length; i < ii; i++ )
      {
        ...
        glyphUnicode = <any>NormalizedUnicodes[glyphUnicode] || glyphUnicode;
    ...
    function addFakeSpaces( width:number, transf?:matrix_t )
    {
    ...
    return new Promise<void>(function promiseBody(resolve, reject) {
      const next = function( promise?:Promise<void> )
      {
      ...
      while( !(stop = timeSlotManager.check()) )
      {
        ...
        switch( fn | 0 ) 
        {
          ...
          case OPS.setLeading:
            ...
            break;
          case OPS.moveText:
            textState.translateTextLineMatrix( <number>args[0], <number>args[1] );
          ...
          case OPS.paintXObject:
            ...
            if( isValidName && emptyXObjectCache.getByName(name) ) break;
              ...
              new Promise<void>( (resolveXObject, rejectXObject) => {
                ...
                self
                  ...
                  .then( () => {
                    ...
                    {
                      emptyXObjectCache.set(name, (<BaseStream>xobj).dict!.objId, true);
                    }
                    resolveXObject();
                  }, rejectXObject);
              }).catch( reason => {
          ...
          case OPS.setGState:
            ...
            next(
              new Promise<void>(function (resolveGState, rejectGState) {
                if (!isValidName) 
                {
                  throw new FormatError("GState must be referred to by name.");
                }
  ...
  extractDataStructures( dict:FontDict, baseDict:FontDict, properties:FontProps )
  {
    const xref = this.xref;
    ...
    if( properties.composite )
    {
      ...
      if( cidSystemInfo instanceof Dict )
      {
        properties.cidSystemInfo = {
          registry: stringToPDFString( <string>(<Dict>cidSystemInfo).get("Registry") ),
          ordering: stringToPDFString( <string>(<Dict>cidSystemInfo).get("Ordering") ),
  ...
  #simpleFontToUnicode( properties:FontProps, forceGlyphs=false ):string[]
  {
    ...
    for( const charcode in encoding )
    {
      ...
      toUnicode[charcode] = String.fromCharCode( <number>glyphsUnicodeMap[glyphName] );
    }
    return toUnicode;
  }
  ...
  async buildToUnicode( properties:FontProps )
  {
    ...
    if (
      ...
      properties.cMap!.forEach( (charcode, cid) => {
        ...
        if (ucs2) {
          toUnicode[charcode] = String.fromCharCode(
            ((<string>ucs2).charCodeAt(0) << 8) + (<string>ucs2).charCodeAt(1)
          );
        }
      });
  ...
  {
    if( !cmapObj ) return Promise.resolve(undefined);
      ...
      return CMapFactory.create({
        ...
        function (cmap) {
          ...
          cmap.forEach( (charCode, token) => {
            ...
            {
              const w1 = ((<string>token).charCodeAt(k) << 8) | (<string>token).charCodeAt(k + 1);
              if ((w1 & 0xf800) !== 0xd800) {
                // w1 < 0xD800 || w1 > 0xDFFF
                str.push(w1);
                continue;
              ...
              k += 2;
              const w2 = ((<string>token).charCodeAt(k) << 8) | (<string>token).charCodeAt(k + 1);
  ...
  readCidToGidMap( glyphsData:Uint8Array | Uint8ClampedArray, toUnicode:IdentityToUnicodeMap | ToUnicodeMap )
  {
    ...
    for( let j = 0, jj = glyphsData.length; j < jj; j++ )
    {
      ...
      const code = j >> 1;
      if( glyphID === 0 && !toUnicode.has(code) ) continue;
  ...
  extractWidths( dict:FontDict, descriptor:FontDict, properties:FontProps )
  {
    ...
    if( properties.composite )
    {
      ...
      {
        const vmetrics_ = <[number,number]>dict.getArray("DW2") ?? [880, -1000];
  ...
  getBaseFontMetrics( name:string ) 
  {
    ...
    if( typeof glyphWidths === "number" )
    {
      ...
      widths = (<() => Record<string, number>>glyphWidths)(); // expand lazy widths array
    }
  ...
  buildCharCodeToWidth( widthsByGlyphName:Record<string,number>, properties:FontProps )
  {
  ...
  preEvaluateFont( dict:FontDict ):PreEvaluatedFont
  {
    ...
    if( descriptor )
    {
      ...
      toUnicode = <BaseStream | Name | undefined>(dict.get("ToUnicode") || baseDict.get("ToUnicode"));
      if( toUnicode instanceof BaseStream )
      {
        const stream = (<DecodeStream>toUnicode).str || toUnicode;
        const uint8array = (<DecodeStream>stream).buffer
          ? new Uint8Array( (<DecodeStream>stream).buffer.buffer, 0, (<DecodeStream>stream).bufferLength )
          : new Uint8Array(
  ...
  async translateFont({
    ...
    let fontFile:BaseStream | undefined,
      subtype:string | undefined,
      length1:number | undefined,
      length2:number | undefined,
      length3:number | undefined;
    try {
      fontFile = <BaseStream|undefined>descriptor.get("FontFile", "FontFile2", "FontFile3");
    } catch (ex) {
      if (!this.options.ignoreErrors) {
        throw ex;
      }
    ...
    properties = {
      ...
      composite,
      fixedPitch: false,
      fontMatrix: <matrix_t|undefined>dict.getArray("FontMatrix") || FONT_IDENTITY_MATRIX,
      firstChar,
      lastChar,
      toUnicode,
      bbox: <rect_t | undefined>(descriptor.getArray("FontBBox") || dict.getArray("FontBBox")),
    ...
    return this.extractDataStructures( dict, baseDict, properties ).then(
      newProperties => {
        ...
        return new Font( (<Name>fontName).name, fontFile, newProperties);
      }
    );
  }

  static buildFontPaths( font:Font, glyphs:Glyph[], 
    handler:MessageHandler<Thread.worker>,
    ...
    function buildPath( fontChar:string )
    {
      ...
      try {
        if( font.renderer.hasBuiltPath(fontChar) ) return;
...
export class TranslatedFont
{
  ...
  #removeType3ColorOperators( operatorList:OperatorList, isEmptyBBox=false )
  {
    // #if !PRODUCTION || TESTING
      assert( operatorList.fnArray[0] === OPS.setCharWidthAndBounds,
    ...
    if (isEmptyBBox) 
    {
      ...
      const charBBox = Util.normalizeRect( 
        <rect_t>(<number[]>operatorList.argsArray[0]).slice(2) 
      );
    ...
    while( i < ii )
    {
      switch (operatorList.fnArray[i]) {
        ...
        case OPS.setGState:
          const [gStateObj] = <[[string][]]>operatorList.argsArray[i];
          let j = 0;
...
export class EvaluatorPreprocessor 
{
  ...
  constructor( stream:BaseStream, xref?:XRef, 
    stateManager:StateManager<EvalState|TextState>=new StateManager(new EvalState()) 
  ) {
