...
// deno-fmt-ignore
const fixedLitCodeTab: FlateTable = [
  new Int32Array([
...
// deno-fmt-ignore
const fixedDistCodeTab: FlateTable = [
  new Int32Array([
...
export class FlateStream extends DecodeStream {
  ...
  override async getImageData(length: uint, _decoderOptions?: DecoderOptions) {
  ...
  readBlock() {
    ...
    try {
      ...
      this.#endsStreamOnError((ex as any).message);
      return;
    }
    if (hdr & 1) {
      this.eof = true;
    ...
    let litCodeTable: FlateTable;
    let distCodeTable: FlateTable;
    if (hdr === 1) {
