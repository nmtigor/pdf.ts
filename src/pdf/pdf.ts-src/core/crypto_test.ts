 ...
 *
 * @module pdf/pdf.ts-src/core/crypto_test.ts
 * @license Apache-2.0
 ******************************************************************************/
...
describe("CipherTransformFactory", () => {
  ...
  function buildDict(map: _DictMap) {
    const dict = new Dict();
    for (const key in map) {
      dict.set(key, map[key as keyof _DictMap]);
    }
    return dict;
  }

  function ensurePasswordCorrect(
    dict: Dict,
    fileId: string,
    password?: string,
  ) {
    try {
      ...
      assertEquals("createCipherTransform" in factory, true);
    } catch {
      // Shouldn't get here.
  ...
  function ensurePasswordNeeded(dict: Dict, fileId: string, password?: string) {
    try {
  ...
  function ensurePasswordIncorrect(
    dict: Dict,
    fileId: string,
    password: string,
  ) {
  ...
  function ensureEncryptDecryptIsIdentity(
    dict: Dict,
    fileId: string,
    password: string,
    string: string,
  ...
  let aes256Dict: Dict,
    aes256IsoDict: Dict,
    aes256BlankDict: Dict,
    aes256IsoBlankDict: Dict;
  ...
  describe("Encrypt and decrypt", () => {
    it("should encrypt and decrypt using ARCFour", () => {
      dict3.CF = buildDict({
        Identity: buildDict({
          CFM: Name.get("V2"),
        } as _DictMap),
    ...
    it("should encrypt and decrypt using AES128", () => {
      dict3.CF = buildDict({
        Identity: buildDict({
          CFM: Name.get("AESV2"),
        } as _DictMap),
    ...
    it("should encrypt and decrypt using AES256", () => {
      dict3.CF = buildDict({
        Identity: buildDict({
          CFM: Name.get("AESV3"),
        } as _DictMap),
    ...
    it("should encrypt and have the correct length using AES128", () => {
      dict3.CF = buildDict({
        Identity: buildDict({
          CFM: Name.get("AESV2"),
        } as _DictMap),
    ...
    it("should encrypt and have the correct length using AES256", () => {
      dict3.CF = buildDict({
        Identity: buildDict({
          CFM: Name.get("AESV3"),
        } as _DictMap),
