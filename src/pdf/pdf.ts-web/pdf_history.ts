...
export class PDFHistory 
{
  ...
  #blockHashChange?:number;
  _currentHash?:string;
  #numPositionUpdates?:number

  #uid?:number;
  #maxUid?:number;
  #destination:PDFHistoryPosition | undefined;
  #position:PDFHistoryPosition | undefined;
  ...
  pushCurrentPosition()
  {
    if( !this.#initialized || this._popStateInProgress ) return;
  ...
  #pushOrReplaceState( destination?:PDFHistoryPosition, forceReplace=false )
  {
  ...
  #tryPushCurrentPosition = ( temporary=false ) =>
  {
    ...
    let forceReplace = false;
    if (
      this.#destination.page! >= position.first! &&
  ...
  #isValidState( state:any, checkReload=false )
  {
    ...
    if (state.fingerprint !== this.#fingerprint) 
    {
      if (checkReload) 
      {
        ...
        if( (<any>perfEntry)?.type !== "reload" ) //kkkk bug? should be `entryType`?
        {
          return false;
        }
      } 
  ...
  #updateInternalState( destination:PDFHistoryPosition | undefined, 
    uid:number, removeTemporary=false 
  ) {
    if (this.#updateViewareaTimeout) {
  ...
  #parseCurrentHash( checkNameddest=false ):PDFHistoryPosition
  {
    const hash = unescape(getCurrentHash()).substring(1);
    const params = parseQueryString(hash);

    const nameddest = <string>params.get("nameddest") || "";
    let page:number | undefined = +params.get("page")! | 0;
  ...
  #updateViewarea = ({ location }:EventMap["updateviewarea"] ) =>
  {
    if( this.#updateViewareaTimeout )
    {
      clearTimeout(this.#updateViewareaTimeout);
