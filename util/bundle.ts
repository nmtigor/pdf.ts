...
if (!PF_) {
  console.log("%cError: No entrypoint provided", "color:red");
  console.log("Provide a js file as entrypoint to bundle.");
  Deno.exit(1);
}
if (!existsSync(PF_, { isReadable: true, isFile: true })) {
  console.log(`%cError: "${PF_}" does not exist.`, "color:red");
  Deno.exit(1);
}
...
if (!PF_1.startsWith(`${D_root}/gen/`)) {
  ...
  console.log(`The entrypoint file must locate in "${D_root}/gen".`);
  Deno.exit(1);
}
const F_ = basename(PF_1);
if (!F_.endsWith(".js")) {
  console.log(`%cError: "${F_}" does not end with ".js".`, "color:red");
  console.log(`The entrypoint file must end with ".js".`);
  Deno.exit(1);
}

const F_0 = F_.slice(0, -extname(F_).length);
// console.log(F_0);
const D_tgt = `${D_root}/built/${
  dirname(PF_1).slice(`${D_root}/gen/`.length)
}`;
// console.log(D_tgt);
...
if (buildResult.errors.length) {
  console.log("%cError: Fail to bundle", "color:red");
  Deno.exit(1);
} else {
